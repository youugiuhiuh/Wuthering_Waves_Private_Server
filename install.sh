...
checkCPUVendor() {
  if [[ -n $(which uname) ]]; then
    if [[ "$(uname)" == "Linux" ]]; then
      case "$(uname -m)" in
      'amd64' | 'x86_64')
        wwpsCoreCPUVendor="Xray-linux-64"
        #                v2rayCoreCPUVendor="v2ray-linux-64"
        warpRegCoreCPUVendor="main-linux-amd64"
        wwpsBoxCoreCPUVendor="-linux-amd64"
        ;;
      'armv8' | 'aarch64')
        cpuVendor="arm"
        wwpsCoreCPUVendor="Xray-linux-arm64-v8a"
        #                v2rayCoreCPUVendor="v2ray-linux-arm64-v8a"
        warpRegCoreCPUVendor="main-linux-arm64"
        wwpsBoxCoreCPUVendor="-linux-arm64"
        ;;
      *)
        echo "  不支持此CPU架构--->"
        exit 1
...
initVar() {
  ...
  wwpsBoxVLESSVisionPort=
  wwpsBoxVLESSRealityVisionPort=
  wwpsBoxVLESSRealityGRPCPort=
  wwpsBoxHysteria2Port=
  wwpsBoxTrojanPort=
  wwpsBoxTuicPort=
  wwpsBoxNaivePort=
  wwpsBoxVMessWSPort=
  wwpsBoxVLESSWSPort=
  wwpsBoxVMessHTTPUpgradePort=
  ...
  # wwps-box reality serverName publicKey
  wwpsBoxVLESSRealityGRPCServerName=
  wwpsBoxVLESSRealityVisionServerName=
  wwpsBoxVLESSRealityPublicKey=

  # wwps-core reality serverName publicKey
  wwpsCoreVLESSRealityServerName=
  wwpsCoreVLESSRealityPort=
  wwpsCoreVLESSRealityXHTTPServerName=
  wwpsCoreVLESSRealityXHTTPort=
  #    wwpsCoreVLESSRealityPublicKey=
...
readNginxSubscribe() {
  ...
  if [[ -f "${nginxConfigPath}subscribe.conf" ]]; then
    if grep -q "wwps-box" "${nginxConfigPath}subscribe.conf"; then
...
readInstallType() {
  ...
  # 1.检测安装目录
  if [[ -d "/etc/wwps" ]]; then
    if [[ -f "/etc/wwps/wwps-core/wwps-core" ]]; then
      # 检测wwps-core
      if [[ -d "/etc/wwps/wwps-core/conf" ]] && [[ -f "/etc/wwps/wwps-core/conf/02_VLESS_TCP_inbounds.json" || -f "/etc/wwps/wwps-core/conf/02_trojan_TCP_inbounds.json" || -f "/etc/wwps/wwps-core/conf/07_VLESS_vision_reality_inbounds.json" ]]; then
        # wwps-core
        configPath=/etc/wwps/wwps-core/conf/
        ctlPath=/etc/wwps/wwps-core/wwps-core
        coreInstallType=1
        if [[ -f "${configPath}07_VLESS_vision_reality_inbounds.json" ]]; then
          realityStatus=1
        fi
        if [[ -f "/etc/wwps/wwps-box/wwps-box" ]] && [[ -f "/etc/wwps/wwps-box/conf/config/06_hysteria2_inbounds.json" || -f "/etc/wwps/wwps-box/conf/config/09_tuic_inbounds.json" || -f "/etc/wwps/wwps-box/conf/config/20_socks5_inbounds.json" ]]; then
          wwpsBoxConfigPath=/etc/wwps/wwps-box/conf/config/
        fi
      fi
    elif [[ -f "/etc/wwps/wwps-box/wwps-box" && -f "/etc/wwps/wwps-box/conf/config.json" ]]; then
      # 检测wwps-box
      ctlPath=/etc/wwps/wwps-box/wwps-box
      coreInstallType=2
      configPath=/etc/wwps/wwps-box/conf/config/
      wwpsBoxConfigPath=/etc/wwps/wwps-box/conf/config/
    fi
  fi
}
  ...
  while read -r row; do
    if echo "${row}" | grep -q VLESS_TCP_inbounds; then
      ...
      if [[ ${coreInstallType} == "2" ]]; then
        wwpsBoxVLESSVisionPort=$(jq .inbounds[0].listen_port "${row}.json")
      fi
    fi
    if echo "${row}" | grep -q VLESS_WS_inbounds; then
      ...
      if [[ ${coreInstallType} == "2" ]]; then
        ...
        wwpsBoxVLESSWSPort=$(jq .inbounds[0].listen_port "${row}.json")
      fi
    fi
    if echo "${row}" | grep -q VLESS_XHTTP_inbounds; then
      currentInstallProtocolType="${currentInstallProtocolType}12,"
      wwpsCoreVLESSRealityXHTTPort=$(jq -r .inbounds[0].port "${row}.json")

      wwpsCoreVLESSRealityXHTTPServerName=$(jq -r .inbounds[0].streamSettings.realitySettings.serverNames[0] "${row}.json")
    ...
    if echo "${row}" | grep -q VMess_WS_inbounds; then
      ...
      if [[ ${coreInstallType} == "2" ]]; then
        ...
        wwpsBoxVMessWSPort=$(jq .inbounds[0].listen_port "${row}.json")
      fi
    fi
    if echo "${row}" | grep -q trojan_TCP_inbounds; then
      ...
      if [[ ${coreInstallType} == "2" ]]; then
        ...
        wwpsBoxTrojanPort=$(jq .inbounds[0].listen_port "${row}.json")
      fi
    fi
    ...
    if echo "${row}" | grep -q hysteria2_inbounds; then
      ...
      if [[ ${coreInstallType} == "2" ]]; then
        ...
        wwpsBoxHysteria2Port=$(jq .inbounds[0].listen_port "${row}.json")
      fi
    fi
    if echo "${row}" | grep -q VLESS_vision_reality_inbounds; then
      currentInstallProtocolType="${currentInstallProtocolType}7,"
      if [[ ${coreInstallType} == "1" ]]; then
        wwpsCoreVLESSRealityServerName=$(jq -r .inbounds[0].streamSettings.realitySettings.serverNames[0] "${row}.json")
        realityServerName=${wwpsCoreVLESSRealityServerName}
        wwpsCoreVLESSRealityPort=$(jq -r .inbounds[0].port "${row}.json")
        ...
        frontingTypeReality=07_VLESS_vision_reality_inbounds
        wwpsBoxVLESSRealityVisionPort=$(jq -r .inbounds[0].listen_port "${row}.json")
        wwpsBoxVLESSRealityVisionServerName=$(jq -r .inbounds[0].tls.server_name "${row}.json")
        realityDomainPort=$(jq -r .inbounds[0].tls.reality.handshake.server_port "${row}.json")

        realityServerName=${wwpsBoxVLESSRealityVisionServerName}
        if [[ -f "${configPath}reality_key" ]]; then
          wwpsBoxVLESSRealityPublicKey=$(grep "publicKey" <"${configPath}reality_key" | awk -F "[:]" '{print $2}')
    ...
    if echo "${row}" | grep -q VLESS_vision_gRPC_inbounds; then
      ...
      if [[ ${coreInstallType} == "2" ]]; then
        ...
        wwpsBoxVLESSRealityGRPCPort=$(jq -r .inbounds[0].listen_port "${row}.json")
        wwpsBoxVLESSRealityGRPCServerName=$(jq -r .inbounds[0].tls.server_name "${row}.json")
        if [[ -f "${configPath}reality_key" ]]; then
          wwpsBoxVLESSRealityPublicKey=$(grep "publicKey" <"${configPath}reality_key" | awk -F "[:]" '{print $2}')
        fi
      fi
    ...
    if echo "${row}" | grep -q tuic_inbounds; then
      ...
      if [[ ${coreInstallType} == "2" ]]; then
        ...
        wwpsBoxTuicPort=$(jq .inbounds[0].listen_port "${row}.json")
      fi
    fi
    if echo "${row}" | grep -q naive_inbounds; then
      ...
      if [[ ${coreInstallType} == "2" ]]; then
        ...
        wwpsBoxNaivePort=$(jq .inbounds[0].listen_port "${row}.json")
      fi
    fi
    if echo "${row}" | grep -q anytls_inbounds; then
      ...
      if [[ ${coreInstallType} == "2" ]]; then
        ...
        wwpsBoxAnyTLSPort=$(jq .inbounds[0].listen_port "${row}.json")
      fi
    fi
    if echo "${row}" | grep -q VMess_HTTPUpgrade_inbounds; then
      ...
      if [[ ${coreInstallType} == "2" ]]; then
        ...
        wwpsBoxVMessHTTPUpgradePort=$(grep 'listen' <${nginxConfigPath}wwps_box_VMess_HTTPUpgrade.conf | awk '{print $2}')
      fi
    fi
    if echo "${row}" | grep -q socks5_inbounds; then
      currentInstallProtocolType="${currentInstallProtocolType}20,"
      wwpsBoxSocks5Port=$(jq .inbounds[0].listen_port "${row}.json")
    fi

  done < <(find ${configPath} -name "*inbounds.json" | sort | awk -F "[.]" '{print $1}')

  if [[ ${coreInstallType} == "1" && -n ${wwpsBoxConfigPath} ]]; then
    if [[ -f "${wwpsBoxConfigPath}06_hysteria2_inbounds.json" ]]; then
      currentInstallProtocolType="${currentInstallProtocolType}6,"
      wwpsBoxHysteria2Port=$(jq .inbounds[0].listen_port "${wwpsBoxConfigPath}06_hysteria2_inbounds.json")
    fi
    if [[ -f "${wwpsBoxConfigPath}09_tuic_inbounds.json" ]]; then
      currentInstallProtocolType="${currentInstallProtocolType}9,"
      wwpsBoxTuicPort=$(jq .inbounds[0].listen_port "${wwpsBoxConfigPath}09_tuic_inbounds.json")
    fi
  fi
    ...
    if [[ -d '/www/server/panel/vhost/cert/' && -n $(find /www/server/panel/vhost/cert/*/fullchain.pem) ]]; then
      ...
      if [[ -n ${selectBTDomain} ]]; then
        ...
        if [[ -z ${btDomain} ]]; then
          ...
          domain=${btDomain}
          if [[ ! -f "/etc/wwps/tls/${btDomain}.crt" && ! -f "/etc/wwps/tls/${btDomain}.key" ]]; then
            ln -s "/www/server/panel/vhost/cert/${btDomain}/fullchain.pem" "/etc/wwps/tls/${btDomain}.crt"
            ln -s "/www/server/panel/vhost/cert/${btDomain}/privkey.pem" "/etc/wwps/tls/${btDomain}.key"
          fi
    ...
    if [[ -d '/opt/1panel/apps/openresty/openresty/www/sites/' && -n $(find /opt/1panel/apps/openresty/openresty/www/sites/*/ssl/fullchain.pem) ]]; then
      ...
      if [[ -n ${selectBTDomain} ]]; then
        ...
        if [[ -z ${btDomain} ]]; then
          ...
          domain=${btDomain}
          if [[ ! -f "/etc/wwps/tls/${btDomain}.crt" && ! -f "/etc/wwps/tls/${btDomain}.key" ]]; then
            ln -s "/opt/1panel/apps/openresty/openresty/www/sites/${btDomain}/ssl/fullchain.pem" "/etc/wwps/tls/${btDomain}.crt"
            ln -s "/opt/1panel/apps/openresty/openresty/www/sites/${btDomain}/ssl/privkey.pem" "/etc/wwps/tls/${btDomain}.key"
          fi
...
allowPort() {
  ...
  if systemctl status firewalld 2>/dev/null | grep -q "active (running)"; then
    ...
    if ! firewall-cmd --list-ports --permanent | grep -qw "${firewallPort}/${type}"; then
      firewall-cmd --zone=public --add-port="${firewallPort}/${type}" --permanent >/dev/null 2>&1
      firewall-cmd --reload >/dev/null 2>&1
      echoContent green " ---> ${firewallPort} 端口开放成功"
    fi
  fi
}
# 获取公网IP
getPublicIP() {
  ...
  if [[ -n ${currentHost} && -z $1 ]] && [[ ${wwpsBoxVLESSRealityVisionServerName} == "${currentHost}" || ${wwpsBoxVLESSRealityGRPCServerName} == "${currentHost}" || ${wwpsCoreVLESSRealityServerName} == "${currentHost}" ]]; then
    echo "${currentHost}"
  else
    local currentIP=
...
readWwpsBoxConfig() {
  hysteriaPort=
  if [[ -n ${wwpsBoxConfigPath} ]]; then

    if [[ -f "${wwpsBoxConfigPath}09_tuic_inbounds.json" ]]; then
      tuicPort=$(jq -r '.inbounds[0].listen_port' "${wwpsBoxConfigPath}09_tuic_inbounds.json")
      tuicAlgorithm=$(jq -r '.inbounds[0].congestion_control' "${wwpsBoxConfigPath}09_tuic_inbounds.json")
    fi
    if [[ -f "${wwpsBoxConfigPath}06_hysteria2_inbounds.json" ]]; then
      hysteriaPort=$(jq -r '.inbounds[0].listen_port' "${wwpsBoxConfigPath}06_hysteria2_inbounds.json")
      hysteria2ClientUploadSpeed=$(jq -r '.inbounds[0].down_mbps' "${wwpsBoxConfigPath}06_hysteria2_inbounds.json")
      hysteria2ClientDownloadSpeed=$(jq -r '.inbounds[0].up_mbps' "${wwpsBoxConfigPath}06_hysteria2_inbounds.json")
    fi
  fi
}
...
unInstallWwpsBox() {
  local type=$1
  if [[ -n ${wwpsBoxConfigPath} ]]; then
    if grep -q 'tuic' </etc/wwps/wwps-box/conf/config.json && [[ ${type} == "tuic" ]]; then
      rm "${wwpsBoxConfigPath}09_tuic_inbounds.json"
      echoContent green " ---> 删除wwps-box tuic配置成功"
    fi

    if grep -q 'hysteria2' </etc/wwps/wwps-box/conf/config.json && [[ ${type} == "hysteria2" ]]; then
      rm "${wwpsBoxConfigPath}06_hysteria2_inbounds.json"
      echoContent green " ---> 删除wwps-box hysteria2配置成功"
    fi
    rm "${wwpsBoxConfigPath}config.json"
  fi

  readInstallType

  if [[ -n ${wwpsBoxConfigPath} ]]; then
    echoContent yellow " ---> 检测到有其他配置，保留wwps-box核心"
    handleWwpsBox stop
    handleWwpsBox start
  else
    handleWwpsBox stop
    rm /etc/systemd/system/wwps-box.service
    rm -rf /etc/wwps/wwps-box/*
    echoContent green " ---> wwps-box 卸载完成"
  fi
}
...
readConfigHostPathUUID() {
  ...
  if [[ ${coreInstallType} == "1" ]]; then
    ...
    if echo ${currentInstallProtocolType} | grep -q ",7,"; then
      ...
      wwpsCoreVLESSRealityVisionPort=$(jq -r .inbounds[0].port ${configPath}07_VLESS_vision_reality_inbounds.json)
      if [[ ${currentPort} == "${wwpsCoreVLESSRealityVisionPort}" ]]; then
        wwpsCoreVLESSRealityVisionPort="${currentDefaultPort}"
      fi
    fi
  elif [[ ${coreInstallType} == "2" ]]; then
    if [[ -n ${frontingType} ]]; then
      ...
      if echo ${currentInstallProtocolType} | grep -q ",11," && [[ ${currentHost} == "null" ]]; then
        currentHost=$(grep 'server_name' <${nginxConfigPath}wwps_box_VMess_HTTPUpgrade.conf | awk '{print $2}')
  ...
  if [[ -n ${configPath} && -n ${frontingType} ]]; then
    ...
    elif [[ ${coreInstallType} == "2" && -f "${wwpsBoxConfigPath}05_VMess_WS_inbounds.json" ]]; then
      wwpsBoxVMessWSPath=$(jq -r .inbounds[0].transport.path "${wwpsBoxConfigPath}05_VMess_WS_inbounds.json")
      currentPath=$(jq -r .inbounds[0].transport.path "${wwpsBoxConfigPath}05_VMess_WS_inbounds.json" | awk -F "[/]" '{print $2}')
    fi
    if [[ ${coreInstallType} == "2" && -f "${wwpsBoxConfigPath}03_VLESS_WS_inbounds.json" ]]; then
      wwpsBoxVLESSWSPath=$(jq -r .inbounds[0].transport.path "${wwpsBoxConfigPath}03_VLESS_WS_inbounds.json")
      currentPath=$(jq -r .inbounds[0].transport.path "${wwpsBoxConfigPath}03_VLESS_WS_inbounds.json" | awk -F "[/]" '{print $2}')
      currentPath=${currentPath::-2}
    fi
    if [[ ${coreInstallType} == "2" && -f "${wwpsBoxConfigPath}11_VMess_HTTPUpgrade_inbounds.json" ]]; then
      wwpsBoxVMessHTTPUpgradePath=$(jq -r .inbounds[0].transport.path "${wwpsBoxConfigPath}11_VMess_HTTPUpgrade_inbounds.json")
      currentPath=$(jq -r .inbounds[0].transport.path "${wwpsBoxConfigPath}11_VMess_HTTPUpgrade_inbounds.json" | awk -F "[/]" '{print $2}')
      # currentPath=${currentPath::-2}
    fi
  fi
  if [[ -f "/etc/wwps/cdn" ]] && [[ -n "$(head -1 /etc/wwps/cdn)" ]]; then
    currentCDNAddress=$(head -1 /etc/wwps/cdn)
  else
...
showInstallStatus() {
  if [[ -n ${coreInstallType} ]]; then
    if [[ ${coreInstallType} == 1 ]]; then
      ...
      if [[ -n $(pgrep -f "wwps-box/wwps-box") ]]; then
        echoContent yellow "\n核心: wwps-box[运行中]"
      else
        echoContent yellow "\n核心: wwps-box[未运行]"
      fi
...
cleanUp() {
  if [[ $1 == "xrayDel" ]]; then
    handleWwpsCore stop
    rm -rf /etc/wwps/wwps-core/*
  elif [[ $1 == "wwpsBoxDel" ]]; then
    handleWwpsBox stop
    rm -rf /etc/wwps/wwps-box/conf/config.json >/dev/null 2>&1
    rm -rf /etc/wwps/wwps-box/conf/config/* >/dev/null 2>&1
  fi
}
initVar "$1"
checkSystem
...
# 初始化安装目录
mkdirTools() {
  mkdir -p /etc/wwps/tls
  mkdir -p /etc/wwps/subscribe_local/default
  mkdir -p /etc/wwps/subscribe_local/clashMeta

  mkdir -p /etc/wwps/subscribe_remote/default
  mkdir -p /etc/wwps/subscribe_remote/clashMeta

  mkdir -p /etc/wwps/subscribe/default
  mkdir -p /etc/wwps/subscribe/clashMetaProfiles
  mkdir -p /etc/wwps/subscribe/clashMeta

  mkdir -p /etc/wwps/subscribe/wwps-box
  mkdir -p /etc/wwps/subscribe/wwps-box_profiles
  mkdir -p /etc/wwps/subscribe_local/wwps-box

  mkdir -p /etc/wwps/wwps-core/conf
  mkdir -p /etc/wwps/wwps-core/reality_scan
  mkdir -p /etc/wwps/wwps-core/tmp
  mkdir -p /etc/systemd/system/
  mkdir -p /tmp/wwps-tls/

  mkdir -p /etc/wwps/warp

  mkdir -p /etc/wwps/wwps-box/conf/config
...
installTools() {
  ...
  if [[ ${release} != "centos" ]]; then
    ${upgrade} >/etc/wwps/install.log 2>&1
  fi

  if grep <"/etc/wwps/install.log" -q "changed"; then
  ...
  if ! command -v firewalld >/dev/null 2>&1; then
    echoContent green " ---> 安装firewalld"
    ${installType} firewalld >/dev/null 2>&1
    bootStartup firewalld
  fi
  ...
  if ! command -v qrencode >/dev/null 2>&1; then
    echoContent yellow "qrencode 安装失败或未找到，TOTP 二维码将无法输出，稍后可手动安装 qrencode 重试"
  fi
  ...
  if [[ ${selectCustomInstallType} == "7" ]]; then
    ...
    if [[ ! -d "$HOME/.acme.sh" ]] || [[ -d "$HOME/.acme.sh" && -z $(find "$HOME/.acme.sh/acme.sh") ]]; then
      ...
      curl -s https://get.acme.sh | sh >/etc/wwps/tls/acme.log 2>&1

      if [[ ! -d "$HOME/.acme.sh" ]] || [[ -z $(find "$HOME/.acme.sh/acme.sh") ]]; then
        echoContent red "  acme安装失败--->"
        tail -n 100 /etc/wwps/tls/acme.log
...
# 检查端口实际开放状态
checkPortOpen() {
  handleWwpsBox stop >/dev/null 2>&1
  handleWwpsCore stop >/dev/null 2>&1
...
initTLSNginxConfig() {
  ...
  if [[ -n ${currentHost} && -z ${lastInstallationConfig} ]]; then
    ...
    if [[ ${historyDomainStatus} == "y" ]]; then
      ...
      echo
      echoContent yellow "请输入要配置的域名 例: www.google.com --->"
      read -r -p "域名:" domain
    fi
...
wwpsBoxNginxConfig() {
  ...
  local wwpsBoxNginxSSL=
  wwpsBoxNginxSSL="ssl_certificate /etc/wwps/tls/${domain}.crt;ssl_certificate_key /etc/wwps/tls/${domain}.key;"
  ...
  if echo "${selectCustomInstallType}" | grep -q ",11," || [[ $1 == "all" ]]; then
    cat <<EOF >>${nginxConfigPath}wwps_box_VMess_HTTPUpgrade.conf
server {
	${nginxH2Conf}
...
switchSSLType() {
  if [[ -z ${sslType} ]]; then
    ...
    *)
    ...
    echo "${sslType}" >/etc/wwps/tls/ssl_type
  fi
}
...
acmeInstallSSL() {
  ...
  if [[ ${dnsAPIType} == "cloudflare" ]]; then
    ...
    sudo CF_Token="${cfAPIToken}" "$HOME/.acme.sh/acme.sh" --issue -d "${dnsAPIDomain}" -d "${dnsTLSDomain}" --dns dns_cf -k ec-256 --server "${sslType}" "${sslIPv6}" 2>&1 | tee -a /etc/wwps/tls/acme.log >/dev/null
  elif [[ ${dnsAPIType} == "aliyun" ]]; then
    echoContent green " --->  DNS API 生成证书中"
    sudo Ali_Key="${aliKey}" Ali_Secret="${aliSecret}" "$HOME/.acme.sh/acme.sh" --issue -d "${dnsAPIDomain}" -d "${dnsTLSDomain}" --dns dns_ali -k ec-256 --server "${sslType}" "${sslIPv6}" 2>&1 | tee -a /etc/wwps/tls/acme.log >/dev/null
  else
    echoContent green " ---> 生成证书中"
    sudo "$HOME/.acme.sh/acme.sh" --issue -d "${tlsDomain}" --standalone -k ec-256 --server "${sslType}" "${sslIPv6}" 2>&1 | tee -a /etc/wwps/tls/acme.log >/dev/null
  fi
}
# 自定义端口
customPortFunction() {
  ...
  if [[ -z ${currentPort} ]] || [[ ${historyCustomPortStatus} == "n" ]]; then
    ...
    if [[ -n ${btDomain} ]]; then
      ...
      if [[ ${port} == "${wwpsCoreVLESSRealityPort}" ]]; then
        handleWwpsCore stop
      fi
    fi
...
installTLS() {
  ...
  # 安装tls
  if [[ -f "/etc/wwps/tls/${tlsDomain}.crt" && -f "/etc/wwps/tls/${tlsDomain}.key" && -n $(cat "/etc/wwps/tls/${tlsDomain}.crt") ]] || [[ -d "$HOME/.acme.sh/${tlsDomain}_ecc" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.key" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.cer" ]] || [[ ${installedDNSAPIStatus} == "true" ]]; then
    echoContent green " ---> 检测到证书"
    renewalTLS

    if [[ -z $(find /etc/wwps/tls/ -name "${tlsDomain}.crt") ]] || [[ -z $(find /etc/wwps/tls/ -name "${tlsDomain}.key") ]] || [[ -z $(cat "/etc/wwps/tls/${tlsDomain}.crt") ]]; then
      if [[ ${installedDNSAPIStatus} == "true" ]]; then
        sudo "$HOME/.acme.sh/acme.sh" --installcert -d "*.${dnsTLSDomain}" --fullchainpath "/etc/wwps/tls/${tlsDomain}.crt" --keypath "/etc/wwps/tls/${tlsDomain}.key" --ecc >/dev/null
      else
        sudo "$HOME/.acme.sh/acme.sh" --installcert -d "${tlsDomain}" --fullchainpath "/etc/wwps/tls/${tlsDomain}.crt" --keypath "/etc/wwps/tls/${tlsDomain}.key" --ecc >/dev/null
      fi
    ...
    if [[ ! -f "/etc/wwps/tls/${tlsDomain}.crt" || ! -f "/etc/wwps/tls/${tlsDomain}.key" ]] || [[ -z $(cat "/etc/wwps/tls/${tlsDomain}.key") || -z $(cat "/etc/wwps/tls/${tlsDomain}.crt") ]]; then
      tail -n 10 /etc/wwps/tls/acme.log
      ...
      if tail -n 10 /etc/wwps/tls/acme.log | grep -q "Could not validate email address as valid"; then
...
nginxBlog() {
  ...
  if [[ -d ${nginxStaticPath} && -f "${nginxStaticPath}/check" ]]; then
    ...
    if [[ ${nginxBlogInstallStatus} == "y" ]]; then
      ...
      if [[ ${release} == "alpine" ]]; then
        wget -q -P "${nginxStaticPath}" "https://raw.githubusercontent.com/SKIPPINGpetticoatconvent/v2ray-agent/master/fodder/blog/unable/html${randomNum}.zip"
      else
        wget -q "${wgetShowProgressStatus}" -P "${nginxStaticPath}" "https://raw.githubusercontent.com/SKIPPINGpetticoatconvent/v2ray-agent/master/fodder/blog/unable/html${randomNum}.zip"
      fi
...
updateSELinuxHTTPPortT() {
  ...
  $(find /usr/bin /usr/sbin | grep -w journalctl) -xe >/etc/wwps/nginx_error.log 2>&1

  if find /usr/bin /usr/sbin | grep -q -w semanage && find /usr/bin /usr/sbin | grep -q -w getenforce && grep -E "31300|31302" </etc/wwps/nginx_error.log | grep -q "Permission denied"; then
...
handleNginx() {
  ...
  if ! echo "${selectCustomInstallType}" | grep -qwE ",7,|,8,|,7,8," && [[ -z $(pgrep -f "nginx") ]] && [[ $1 == "start" ]]; then
    if [[ ${release} == "alpine" ]]; then
      rc-service nginx start 2>/etc/wwps/nginx_error.log
    else
      systemctl start nginx 2>/etc/wwps/nginx_error.log
    fi
    ...
    if [[ -z $(pgrep -f "nginx") ]]; then
      ...
      nginx
      if grep -q "journalctl -xe" </etc/wwps/nginx_error.log; then
        updateSELinuxHTTPPortT
      fi
    else
...
installCronTLS() {
  if [[ -z ${btDomain} ]]; then
    ...
    crontab -l >/etc/wwps/backup_crontab.cron
    local historyCrontab
    historyCrontab=$(sed '/wwps/d;/acme.sh/d' /etc/wwps/backup_crontab.cron)
    echo "${historyCrontab}" >/etc/wwps/backup_crontab.cron
    echo "30 1 * * * /bin/bash /etc/wwps/install.sh RenewTLS >> /etc/wwps/crontab_tls.log 2>&1" >>/etc/wwps/backup_crontab.cron
    crontab /etc/wwps/backup_crontab.cron
    echoContent green "\n ---> 添加定时维护证书成功"
  fi
}
# 定时任务更新geo文件
installCronUpdateGeo() {
  if [[ ${coreInstallType} == "1" ]]; then
    ...
    echoContent skyBlue "\n进度 1/1 : 添加定时更新geo文件"
    crontab -l >/etc/wwps/backup_crontab.cron
    echo "35 1 * * * /bin/bash /etc/wwps/install.sh UpdateGeo >> /etc/wwps/crontab_tls.log 2>&1" >>/etc/wwps/backup_crontab.cron
    crontab /etc/wwps/backup_crontab.cron
    echoContent green "\n ---> 添加定时更新geo文件成功"
  fi
}
...
renewalTLS() {
  ...
  if [[ -f "/etc/wwps/tls/ssl_type" ]]; then
    if grep -q "buypass" <"/etc/wwps/tls/ssl_type"; then
      sslRenewalDays=180
    fi
  fi
  if [[ -d "$HOME/.acme.sh/${domain}_ecc" && -f "$HOME/.acme.sh/${domain}_ecc/${domain}.key" && -f "$HOME/.acme.sh/${domain}_ecc/${domain}.cer" ]] || [[ ${installedDNSAPIStatus} == "true" ]]; then
    ...
    if [[ ${remainingDays} -le 1 ]]; then
      ...
      sudo "$HOME/.acme.sh/acme.sh" --installcert -d "${domain}" --fullchainpath /etc/wwps/tls/"${domain}.crt" --keypath /etc/wwps/tls/"${domain}.key" --ecc
      reloadCore
      handleNginx start
    else
      echoContent green " ---> 证书有效"
    fi
  elif [[ -f "/etc/wwps/tls/${tlsDomain}.crt" && -f "/etc/wwps/tls/${tlsDomain}.key" && -n $(cat "/etc/wwps/tls/${tlsDomain}.crt") ]]; then
...
installWwpsBox() {
  readInstallType
  echoContent skyBlue "\n进度  $1/${totalProgress} : 安装wwps-box"

  if [[ ! -f "/etc/wwps/wwps-box/wwps-box" ]]; then
    ...
    if [[ ${release} == "alpine" ]]; then
      wget -c -q -P /etc/wwps/wwps-box/ "https://github.com/SagerNet/sing-box/releases/download/${version}/sing-box-${version/v/}${wwpsBoxCoreCPUVendor}.tar.gz"
    else
      wget -c -q "${wgetShowProgressStatus}" -P /etc/wwps/wwps-box/ "https://github.com/SagerNet/sing-box/releases/download/${version}/sing-box-${version/v/}${wwpsBoxCoreCPUVendor}.tar.gz"
    fi

    if [[ ! -f "/etc/wwps/wwps-box/sing-box-${version/v/}${wwpsBoxCoreCPUVendor}.tar.gz" ]]; then
      ...
      tar zxvf "/etc/wwps/wwps-box/sing-box-${version/v/}${wwpsBoxCoreCPUVendor}.tar.gz" -C "/etc/wwps/wwps-box/" >/dev/null 2>&1

      mv "/etc/wwps/wwps-box/sing-box-${version/v/}${wwpsBoxCoreCPUVendor}/sing-box" /etc/wwps/wwps-box/wwps-box
      rm -rf /etc/wwps/wwps-box/sing-box-*
      chmod 655 /etc/wwps/wwps-box/wwps-box
    fi
  else
    echoContent green " ---> 当前版本:v$(/etc/wwps/wwps-box/wwps-box version | grep "wwps-box version" | awk '{print $3}')"

    version=$(curl -s "https://api.github.com/repos/SagerNet/sing-box/releases?per_page=20" | jq -r ".[]|select (.prerelease==${prereleaseStatus})|.tag_name" | head -1)
    echoContent green " ---> 最新版本:${version}"

    if [[ -z ${lastInstallationConfig} ]]; then
      read -r -p "是否更新、升级？[y/n]:" reInstallWwpsBoxStatus
      if [[ ${reInstallWwpsBoxStatus} == "y" ]]; then
        rm -f /etc/wwps/wwps-box/wwps-box
        installWwpsBox "$1"
      fi
    fi
  fi
...
installWwpsCore() {
  ...
  echoContent skyBlue "\n进度  $1/${totalProgress} : 安装wwps-core"
  ...
  if [[ ! -f "/etc/wwps/wwps-core/wwps-core" ]]; then
    ...
    echoContent green " ---> wwps-core版本:${version}"
    if [[ ${release} == "alpine" ]]; then
      wget -c -q -P /etc/wwps/wwps-core/ "https://github.com/XTLS/Xray-core/releases/download/${version}/${wwpsCoreCPUVendor}.zip"
    else
      wget -c -q "${wgetShowProgressStatus}" -P /etc/wwps/wwps-core/ "https://github.com/XTLS/Xray-core/releases/download/${version}/${wwpsCoreCPUVendor}.zip"
    fi

    if [[ ! -f "/etc/wwps/wwps-core/${wwpsCoreCPUVendor}.zip" ]]; then
      read -r -p "核心下载失败，请重新尝试安装，是否重新尝试？[y/n]" downloadStatus
      if [[ ${downloadStatus} == "y" ]]; then
        installWwpsCore "$1"
      fi
    else
      unzip -o "/etc/wwps/wwps-core/${wwpsCoreCPUVendor}.zip" -d /etc/wwps/wwps-core >/dev/null
      rm -rf "/etc/wwps/wwps-core/${wwpsCoreCPUVendor}.zip"
      mv /etc/wwps/wwps-core/xray /etc/wwps/wwps-core/wwps-core

      version=$(curl -s https://api.github.com/repos/Loyalsoldier/v2ray-rules-dat/releases?per_page=1 | jq -r '.[]|.tag_name')
      echoContent skyBlue "------------------------Version-------------------------------"
      echo "version:${version}"
      rm /etc/wwps/wwps-core/geo* >/dev/null 2>&1

      if [[ ${release} == "alpine" ]]; then
        wget -c -q -P /etc/wwps/wwps-core/ "https://github.com/Loyalsoldier/v2ray-rules-dat/releases/download/${version}/geosite.dat"
        wget -c -q -P /etc/wwps/wwps-core/ "https://github.com/Loyalsoldier/v2ray-rules-dat/releases/download/${version}/geoip.dat"
      else
        wget -c -q "${wgetShowProgressStatus}" -P /etc/wwps/wwps-core/ "https://github.com/Loyalsoldier/v2ray-rules-dat/releases/download/${version}/geosite.dat"
        wget -c -q "${wgetShowProgressStatus}" -P /etc/wwps/wwps-core/ "https://github.com/Loyalsoldier/v2ray-rules-dat/releases/download/${version}/geoip.dat"
      fi

      chmod 655 /etc/wwps/wwps-core/wwps-core
    fi
  else
    if [[ -z ${lastInstallationConfig} ]]; then
      echoContent green " ---> wwps-core版本:$(/etc/wwps/wwps-core/wwps-core --version | awk '{print $2}' | head -1)"
      read -r -p "是否更新、升级？[y/n]:" reInstallWwpsCoreStatus
      if [[ ${reInstallWwpsCoreStatus} == "y" ]]; then
        rm -f /etc/wwps/wwps-core/wwps-core
        installWwpsCore "$1" "$2"
      fi
    fi
  fi
...
# wwps-core版本管理
wwpsCoreVersionManageMenu() {
  echoContent skyBlue "\n进度  $1/${totalProgress} : wwps-core版本管理"
  if [[ ${coreInstallType} != "1" ]]; then
    echoContent red " ---> 没有检测到安装目录，请执行脚本安装内容"
    exit 0
  fi
  echoContent red "\n=============================================================="
  echoContent yellow "1.升级wwps-core"
  echoContent yellow "2.升级wwps-core 预览版"
  echoContent yellow "3.回退wwps-core"
  echoContent yellow "4.关闭wwps-core"
  echoContent yellow "5.打开wwps-core"
  echoContent yellow "6.重启wwps-core"
  echoContent yellow "7.更新geosite、geoip"
  echoContent yellow "8.设置自动更新geo文件[每天凌晨更新]"
  echoContent yellow "9.查看日志"
  echoContent red "=============================================================="
  read -r -p "请选择:" selectWwpsCoreType
  if [[ ${selectWwpsCoreType} == "1" ]]; then
    prereleaseStatus=false
    updateWwpsCore
  elif [[ ${selectWwpsCoreType} == "2" ]]; then
    prereleaseStatus=true
    updateWwpsCore
  elif [[ ${selectWwpsCoreType} == "3" ]]; then
    echoContent yellow "\n1.只可以回退最近的五个版本"
    ...
    echoContent skyBlue "--------------------------------------------------------------"
    read -r -p "请输入要回退的版本:" selectWwpsCoreVersionType
    version=$(curl -s "https://api.github.com/repos/XTLS/Xray-core/releases?per_page=5" | jq -r ".[]|select (.prerelease==false)|.tag_name" | awk '{print ""NR""":"$0}' | grep "${selectWwpsCoreVersionType}:" | awk -F "[:]" '{print $2}')
    if [[ -n ${version} ]]; then
      updateWwpsCore "${version}"
    else
      echoContent red "\n ---> 输入有误，请重新输入"
      wwpsCoreVersionManageMenu 1
    fi
  elif [[ ${selectWwpsCoreType} == "4" ]]; then
    handleWwpsCore stop
  elif [[ ${selectWwpsCoreType} == "5" ]]; then
    handleWwpsCore start
  elif [[ ${selectWwpsCoreType} == "6" ]]; then
    reloadCore
  elif [[ ${selectWwpsCoreType} == "7" ]]; then
    updateGeoSite
  elif [[ ${selectWwpsCoreType} == "8" ]]; then
    installCronUpdateGeo
  elif [[ ${selectWwpsCoreType} == "9" ]]; then
    checkLog 1
  fi
}
...
updateWwpsCore() {
  ...
  if [[ -z ${coreInstallType} || ${coreInstallType} != "1" ]]; then
    ...
    echoContent green " ---> 当前版本:v$(/etc/wwps/wwps-core/wwps-core --version | awk '{print $2}' | head -1)"
    ...
    if [[ -n $1 ]]; then
      read -r -p "回退版本为${version}，是否继续？[y/n]:" rollbackWwpsCoreStatus
      if [[ ${rollbackWwpsCoreStatus} == "y" ]]; then
        echoContent green " ---> 当前wwps-core版本:$(/etc/wwps/wwps-core/wwps-core --version | awk '{print $2}' | head -1)"
    ...
    elif [[ ${version} == "v$(/etc/wwps/wwps-core/wwps-core --version | awk '{print $2}' | head -1)" ]]; then
      read -r -p "当前版本与最新版相同，是否重新安装？[y/n]:" reInstallWwpsCoreStatus
      if [[ ${reInstallWwpsCoreStatus} == "y" ]]; then
        handleWwpsCore stop
        rm -f /etc/wwps/wwps-core/wwps-core
        updateWwpsCore
      ...
      read -r -p "最新版本为:${version}，是否更新？[y/n]:" installWwpsCoreStatus
      if [[ ${installWwpsCoreStatus} == "y" ]]; then
        rm /etc/wwps/wwps-core/wwps-core
        updateWwpsCore
      else
...
checkGFWStatue() {
  ...
  if [[ ${coreInstallType} == "1" ]] && [[ -n $(pgrep -f "wwps-core") ]]; then
    echoContent green " ---> 服务启动成功"
  elif [[ ${coreInstallType} == "2" ]] && [[ -n $(pgrep -f "wwps-box/wwps-box") ]]; then
    echoContent green " ---> 服务启动成功"
  else
...
installAlpineStartup() {
  local serviceName=$1
  if [[ ${serviceName} == "wwps-box" ]]; then
    cat <<EOF >"/etc/init.d/${serviceName}"
#!/sbin/openrc-run

description="wwps-box service"
command="/etc/wwps/wwps-box/wwps-box"
command_args="run -c /etc/wwps/wwps-box/conf/config.json"
command_background=true
pidfile="/var/run/wwps-box.pid"
EOF
  elif [[ ${serviceName} == "wwps-core" ]]; then
    cat <<EOF >"/etc/init.d/${serviceName}"
#!/sbin/openrc-run

description="wwps-core service"
command="/etc/wwps/wwps-core/wwps-core"
command_args="run -confdir /etc/wwps/wwps-core/conf"
command_background=true
pidfile="/var/run/wwps-core.pid"
EOF
  fi
...
# wwps-box开机自启
installWwpsBoxService() {
  echoContent skyBlue "\n进度  $1/${totalProgress} : 配置wwps-box开机自启"
  execStart='/etc/wwps/wwps-box/wwps-box run -c /etc/wwps/wwps-box/conf/config.json'

  if [[ -n $(find /bin /usr/bin -name "systemctl") && ${release} != "alpine" ]]; then
    rm -rf /etc/systemd/system/wwps-box.service
    touch /etc/systemd/system/wwps-box.service
    cat <<EOF >/etc/systemd/system/wwps-box.service
[Unit]
Description=wwps-box Service
Documentation=https://wwps-box.sagernet.org
...
EOF
    bootStartup "wwps-box.service"
  elif [[ ${release} == "alpine" ]]; then
    installAlpineStartup "wwps-box"
    bootStartup "wwps-box"
  fi

  echoContent green " ---> 配置wwps-box开机启动完毕"
}

# wwps-core开机自启
installWwpsCoreService() {
  echoContent skyBlue "\n进度  $1/${totalProgress} : 配置wwps-core开机自启"
  execStart='/etc/wwps/wwps-core/wwps-core run -confdir /etc/wwps/wwps-core/conf'
  if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
    rm -rf /etc/systemd/system/wwps-core.service
    touch /etc/systemd/system/wwps-core.service
    cat <<EOF >/etc/systemd/system/wwps-core.service
[Unit]
Description=wwps-core Service
...
WantedBy=multi-user.target
EOF
    bootStartup "wwps-core.service"
    echoContent green " ---> 配置wwps-core开机自启成功"
  elif [[ ${release} == "alpine" ]]; then
    installAlpineStartup "wwps-core"
    bootStartup "wwps-core"
  fi
}
...
handleHysteria() {
  ...
  if [[ $1 == "start" ]]; then
    if [[ -n $(pgrep -f "hysteria/hysteria") ]]; then
      ...
      echoContent red "请手动执行【/etc/wwps/hysteria/hysteria --log-level debug -c /etc/wwps/hysteria/conf/config.json server】，查看错误日志"
      exit 0
    fi
  elif [[ $1 == "stop" ]]; then
...
# 操作wwps-box
handleWwpsBox() {
  if [[ -f "/etc/systemd/system/wwps-box.service" ]]; then
    if [[ -z $(pgrep -f "wwps-box") ]] && [[ $1 == "start" ]]; then
      wwpsBoxMergeConfig
      systemctl start wwps-box.service
    elif [[ -n $(pgrep -f "wwps-box") ]] && [[ $1 == "stop" ]]; then
      systemctl stop wwps-box.service
    fi
  elif [[ -f "/etc/init.d/wwps-box" ]]; then
    if [[ -z $(pgrep -f "wwps-box") ]] && [[ $1 == "start" ]]; then
      wwpsBoxMergeConfig
      rc-service wwps-box start
    elif [[ -n $(pgrep -f "wwps-box") ]] && [[ $1 == "stop" ]]; then
      rc-service wwps-box stop
    fi
  fi
  sleep 1

  if [[ $1 == "start" ]]; then
    if [[ -n $(pgrep -f "wwps-box") ]]; then
      echoContent green " ---> wwps-box启动成功"
    else
      echoContent red "wwps-box启动失败"
      echoContent yellow "请手动执行【 /etc/wwps/wwps-box/wwps-box merge config.json -C /etc/wwps/wwps-box/conf/config/ -D /etc/wwps/wwps-box/conf/ 】，查看错误日志"
      echo
      echoContent yellow "如上面命令没有错误，请手动执行【 /etc/wwps/wwps-box/wwps-box run -c /etc/wwps/wwps-box/conf/config.json 】，查看错误日志"
      exit 0
    fi
  elif [[ $1 == "stop" ]]; then
    if [[ -z $(pgrep -f "wwps-box") ]]; then
      echoContent green " ---> wwps-box关闭成功"
    else
      echoContent red " ---> wwps-box关闭失败"
      echoContent red "请手动执行【ps -ef|grep -v grep|grep wwps-box|awk '{print \$2}'|xargs kill -9】"
      exit 0
    fi
  fi
}
...
# 操作wwps-core
handleWwpsCore() {
  if [[ -n $(find /bin /usr/bin -name "systemctl") ]] && [[ -n $(find /etc/systemd/system/ -name "wwps-core.service") ]]; then
    if [[ -z $(pgrep -f "wwps-core") ]] && [[ $1 == "start" ]]; then
      systemctl start wwps-core.service
    elif [[ -n $(pgrep -f "wwps-core") ]] && [[ $1 == "stop" ]]; then
      systemctl stop wwps-core.service
    fi
  elif [[ -f "/etc/init.d/wwps-core" ]]; then
    if [[ -z $(pgrep -f "wwps-core") ]] && [[ $1 == "start" ]]; then
      rc-service wwps-core start
    elif [[ -n $(pgrep -f "wwps-core") ]] && [[ $1 == "stop" ]]; then
      rc-service wwps-core stop
    fi
  fi
  ...
  if [[ $1 == "start" ]]; then
    if [[ -n $(pgrep -f "wwps-core") ]]; then
      echoContent green " ---> wwps-core启动成功"
    else
      echoContent red "wwps-core启动失败"
      echoContent red "请手动执行以下的命令后【/etc/wwps/wwps-core/wwps-core -confdir /etc/wwps/wwps-core/conf】将错误日志进行反馈"
      exit 0
    fi
  elif [[ $1 == "stop" ]]; then
    if [[ -z $(pgrep -f "wwps-core") ]]; then
      echoContent green " ---> wwps-core关闭成功"
    else
      echoContent red "wwps-core关闭失败"
      echoContent red "请手动执行【ps -ef|grep -v grep|grep wwps-core|awk '{print \$2}'|xargs kill -9】"
      exit 0
    fi
  fi
}
  ...
  if [[ -z ${portHoppingRange} ]]; then
    ...
    if [[ -z ${portStart} || -z ${portEnd} ]]; then
      ...
      if [[ ${release} == "centos" ]]; then
        ...
        iptables -t nat -A PREROUTING -p udp --dport "${portStart}:${portEnd}" -m comment --comment "SKIPPINGpetticoatconvent_${type}_portHopping" -j DNAT --to-destination ":${targetPort}"
        sudo netfilter-persistent save
        if ! iptables-save | grep -q "SKIPPINGpetticoatconvent_${type}_portHopping"; then
          echoContent red " ---> 端口跳跃添加失败"
          exit 0
        fi
      fi
...
readPortHopping() {
  ...
  if [[ ${release} == "centos" ]]; then
    ...
    if iptables-save | grep -q "SKIPPINGpetticoatconvent_${type}_portHopping"; then
      local portHopping=
      portHopping=$(iptables-save | grep "SKIPPINGpetticoatconvent_${type}_portHopping" | cut -d " " -f 8)
...
deletePortHoppingRules() {
  ...
  if [[ ${release} == "centos" ]]; then
    ...
    iptables -t nat -L PREROUTING --line-numbers | grep "SKIPPINGpetticoatconvent_${type}_portHopping" | awk '{print $1}' | while read -r line; do
...
portHoppingMenu() {
  ...
  if [[ ${type} == "hysteria2" ]]; then
    readPortHopping "${type}" "${wwpsBoxHysteria2Port}"
    targetPort=${wwpsBoxHysteria2Port}
    portHoppingStart=${hysteria2PortHoppingStart}
    portHoppingEnd=${hysteria2PortHoppingEnd}
  elif [[ ${type} == "tuic" ]]; then
    readPortHopping "${type}" "${wwpsBoxTuicPort}"
    targetPort=${wwpsBoxTuicPort}
...
#    initTuicPort
#    initTuicProtocol
#    cat <<EOF >/etc/wwps/tuic/conf/config.json
#{
#    "server": "[::]:${tuicPort}",
#    "users": $(initWwpsCoreClients 9),
#    "certificate": "/etc/wwps/tls/${currentHost}.crt",
#    "private_key": "/etc/wwps/tls/${currentHost}.key",
...
# 初始化singbox route配置
initWwpsBoxRouteConfig() {
  downloadWwpsBoxGeositeDB
  local outboundTag=$1
  if [[ ! -f "${wwpsBoxConfigPath}${outboundTag}_route.json" ]]; then
    cat <<EOF >"${wwpsBoxConfigPath}${outboundTag}_route.json"
{
    "route": {
        "geosite": {
            "path": "${wwpsBoxConfigPath}geosite.db"
        },
        "rules": [
            {
                "domain": [
...
# 下载wwps-box geosite db
downloadWwpsBoxGeositeDB() {
  if [[ ! -f "${wwpsBoxConfigPath}geosite.db" ]]; then
    if [[ ${release} == "alpine" ]]; then
      wget -q -P "${wwpsBoxConfigPath}" https://github.com/Johnshall/sing-geosite/releases/latest/download/geosite.db
    else
      wget -q "${wgetShowProgressStatus}" -P "${wwpsBoxConfigPath}" https://github.com/Johnshall/sing-geosite/releases/latest/download/geosite.db
    fi
...
addWwpsBoxRouteRule() {
  ...
  local routingName=$3
  # 读取上次安装内容
  if [[ -f "${wwpsBoxConfigPath}${routingName}.json" ]]; then
    read -r -p "读取到上次的配置，是否保留 ？[y/n]:" historyRouteStatus
    if [[ ${historyRouteStatus} == "y" ]]; then
      domainList="${domainList},$(jq -rc .route.rules[0].rule_set[] "${wwpsBoxConfigPath}${routingName}.json" | awk -F "[_]" '{print $1}' | paste -sd ',')"
      domainList="${domainList},$(jq -rc .route.rules[0].domain_regex[] "${wwpsBoxConfigPath}${routingName}.json" | awk -F "[*]" '{print $2}' | paste -sd ',' | sed 's/\\//g')"
    fi
  fi
  local rules=
  rules=$(initWwpsBoxRules "${domainList}" "${routingName}")
  # domain精确匹配规则
  local domainRules=
  ...
  if [[ -n ${wwpsBoxConfigPath} ]]; then
    ...
    cat <<EOF >"${wwpsBoxConfigPath}${routingName}.json"
{
  "route": {
    "rules": [
      {
...
EOF
    jq 'if .route.rule_set == [] then del(.route.rule_set) else . end' "${wwpsBoxConfigPath}${routingName}.json" >"${wwpsBoxConfigPath}${routingName}_tmp.json" && mv "${wwpsBoxConfigPath}${routingName}_tmp.json" "${wwpsBoxConfigPath}${routingName}.json"
  fi

}

# 移除wwps-box route rule
removeWwpsBoxRouteRule() {
  local outboundTag=$1
  local delRules
  if [[ -f "${wwpsBoxConfigPath}${outboundTag}_route.json" ]]; then
    delRules=$(jq -r 'del(.route.rules[]|select(.outbound=="'"${outboundTag}"'"))' "${wwpsBoxConfigPath}${outboundTag}_route.json")
    echo "${delRules}" >"${wwpsBoxConfigPath}${outboundTag}_route.json"
  fi
}
...
addWwpsBoxOutbound() {
  ...
  if [[ -n ${detour} ]]; then
    cat <<EOF >"${wwpsBoxConfigPath}${tag}.json"
{
     "outbounds": [
        {
             "type": "direct",
...
addWwpsCoreOutbound() {
  ...
  if [[ -n ${domainStrategy} ]]; then
    cat <<EOF >"/etc/wwps/wwps-core/conf/${tag}.json"
{
    "outbounds":[
        {
            "protocol":"freedom",
...
# 删除 wwps-core出站
removeWwpsCoreOutbound() {
  local tag=$1
  if [[ -f "/etc/wwps/wwps-core/conf/${tag}.json" ]]; then
    rm "/etc/wwps/wwps-core/conf/${tag}.json" >/dev/null 2>&1
  fi
}
# 移除wwps-box配置
removeWwpsBoxConfig() {
  ...
  local tag=$1
  if [[ -f "${wwpsBoxConfigPath}${tag}.json" ]]; then
    rm "${wwpsBoxConfigPath}${tag}.json"
  fi
}

# 初始化wireguard出站信息
addWwpsBoxWireGuardEndpoints() {
  local type=$1

  readConfigWarpReg

  cat <<EOF >"${wwpsBoxConfigPath}wireguard_endpoints_${type}.json"
{
     "endpoints": [
        {
            "type": "wireguard",
...
# 初始化 wwps-box Hysteria2 配置
initWwpsBoxHysteria2Config() {
  echoContent skyBlue "\n进度 $1/${totalProgress} : 初始化Hysteria2配置"

  initHysteriaPort
  initHysteria2Network

  cat <<EOF >/etc/wwps/wwps-box/conf/config/hysteria2.json
{
    "inbounds": [
        {
            "type": "hysteria2",
            ...
            "tls": {
                ...
                "certificate_path": "/etc/wwps/tls/${currentHost}.crt",
                "key_path": "/etc/wwps/tls/${currentHost}.key"
            }
        }
    ]
}
...
wwpsBoxTuicInstall() {
  ...
  initWwpsBoxConfig custom 2 true
  installWwpsBoxService 3
  reloadCore
  showAccounts 4
}

# wwps-box hy2安装
wwpsBoxHysteria2Install() {
  if ! echo "${currentInstallProtocolType}" | grep -qE ",0,|,1,|,2,|,3,|,4,|,5,|,6,|,9,|,10,"; then
...
# 合并config
wwpsBoxMergeConfig() {
  rm /etc/wwps/wwps-box/conf/config.json >/dev/null 2>&1
  /etc/wwps/wwps-box/wwps-box merge config.json -C /etc/wwps/wwps-box/conf/config/ -D /etc/wwps/wwps-box/conf/ >/dev/null 2>&1
}

# 初始化Xray Trojan XTLS 配置文件
#initWwpsCoreFrontingConfig() {
...
initWwpsCoreConfig() {
  ...
  if [[ -z ${addClientsStatus} ]]; then
    ...
    if [[ -n ${customUUID} ]]; then
      ...
      uuid=$(/etc/wwps/wwps-core/wwps-core uuid)
    fi
  ...
  # log
  if [[ ! -f "/etc/wwps/wwps-core/conf/00_log.json" ]]; then

    cat <<EOF >/etc/wwps/wwps-core/conf/00_log.json
{
  "log": {
    "error": "/etc/wwps/wwps-core/error.log",
    "loglevel": "warning",
    "dnsLog": false
  }
}
EOF
  ...
  if [[ ! -f "/etc/wwps/wwps-core/conf/12_policy.json" ]]; then

    cat <<EOF >/etc/wwps/wwps-core/conf/12_policy.json
{
  "policy": {
      "levels": {
          "0": {
...
EOF
  ...
  # dns
  if [[ ! -f "/etc/wwps/wwps-core/conf/11_dns.json" ]]; then
    cat <<EOF >/etc/wwps/wwps-core/conf/11_dns.json
{
    "dns": {
        "servers": [
          "localhost"
...
EOF
  ...
  # routing
  cat <<EOF >/etc/wwps/wwps-core/conf/09_routing.json
{
  "routing": {
    "rules": [
      {
...
EOF
  ...
  if echo "${selectCustomInstallType}" | grep -q ",4," || [[ $1 == "all" ]]; then
    ...
    cat <<EOF >/etc/wwps/wwps-core/conf/04_trojan_TCP_inbounds.json
{
"inbounds":[
	{
	  "port": 31296,
...
EOF
    ...
    rm /etc/wwps/wwps-core/conf/04_trojan_TCP_inbounds.json >/dev/null 2>&1
  fi

  # VLESS_WS_TLS
  if echo "${selectCustomInstallType}" | grep -q ",1," || [[ $1 == "all" ]]; then
    fallbacksList=${fallbacksList}',{"path":"/'${customPath}'ws","dest":31297,"xver":1}'
    cat <<EOF >/etc/wwps/wwps-core/conf/03_VLESS_WS_inbounds.json
{
"inbounds":[
    {
	  "port": 31297,
...
EOF
    ...
    rm /etc/wwps/wwps-core/conf/03_VLESS_WS_inbounds.json >/dev/null 2>&1
  fi
  # VLESS_Reality_XHTTP_TLS
  if echo "${selectCustomInstallType}" | grep -q ",12," || [[ $1 == "all" ]]; then
    initWwpsCoreXHTTPort
    initRealityClientServersName
    initRealityKey
    initRealityMldsa65
    cat <<EOF >/etc/wwps/wwps-core/conf/12_VLESS_XHTTP_inbounds.json
{
"inbounds":[
    {
	  "port": ${xHTTPort},
	  ...
	  "settings": {
		"clients": $(initWwpsCoreClients 12),
		"decryption": "none"
	  },
	  "streamSettings": {
		"network": "xhttp",
...
EOF
    ...
    rm /etc/wwps/wwps-core/conf/12_VLESS_XHTTP_inbounds.json >/dev/null 2>&1
  fi
  if echo "${selectCustomInstallType}" | grep -q ",3," || [[ $1 == "all" ]]; then
    fallbacksList=${fallbacksList}',{"path":"/'${customPath}'vws","dest":31299,"xver":1}'
    cat <<EOF >/etc/wwps/wwps-core/conf/05_VMess_WS_inbounds.json
{
    "inbounds":[
        {
          "listen": "127.0.0.1",
...
EOF
    ...
    rm /etc/wwps/wwps-core/conf/05_VMess_WS_inbounds.json >/dev/null 2>&1
  fi
  # VLESS_gRPC
  #    if echo "${selectCustomInstallType}" | grep -q ",5," || [[ "$1" == "all" ]]; then
  #        cat <<EOF >/etc/wwps/wwps-core/conf/06_VLESS_gRPC_inbounds.json
  #{
  #    "inbounds":[
  #        {
  #            "port": 31301,
  ...
  #EOF
  #    elif [[ -z "$3" ]]; then
  #        rm /etc/wwps/wwps-core/conf/06_VLESS_gRPC_inbounds.json >/dev/null 2>&1
  #    fi

  # VLESS Vision
  if echo "${selectCustomInstallType}" | grep -q ",0," || [[ $1 == "all" ]]; then

    cat <<EOF >/etc/wwps/wwps-core/conf/02_VLESS_TCP_inbounds.json
{
    "inbounds":[
        {
          "port": ${port},
          ...
          "streamSettings": {
            ...
            "tlsSettings": {
              ...
              "certificates": [
                {
                  "certificateFile": "/etc/wwps/tls/${domain}.crt",
                  "keyFile": "/etc/wwps/tls/${domain}.key",
                  "ocspStapling": 3600
                }
              ]
            }
...
EOF
    ...
    rm /etc/wwps/wwps-core/conf/02_VLESS_TCP_inbounds.json >/dev/null 2>&1
  fi
  ...
  if echo "${selectCustomInstallType}" | grep -q ",7," || [[ $1 == "all" ]]; then
    ...
    initWwpsCoreRealityPort
    initRealityClientServersName
    initRealityKey
    initRealityMldsa65
    cat <<EOF >/etc/wwps/wwps-core/conf/07_VLESS_vision_reality_inbounds.json
{
  "inbounds": [
    {
      "listen": "0.0.0.0",
...
EOF
    #        cat <<EOF >/etc/wwps/wwps-core/conf/08_VLESS_vision_gRPC_inbounds.json
    #{
    #  "inbounds": [
    #    {
    #      "port": 31305,
    ...
    rm /etc/wwps/wwps-core/conf/07_VLESS_vision_reality_inbounds.json >/dev/null 2>&1
    rm /etc/wwps/wwps-core/conf/08_VLESS_vision_gRPC_inbounds.json >/dev/null 2>&1
  fi
  installSniffing
  if [[ -z $3 ]]; then
    removeWwpsCoreOutbound wireguard_out_IPv4_route
    removeWwpsCoreOutbound wireguard_out_IPv6_route
    removeWwpsCoreOutbound wireguard_outbound
    removeWwpsCoreOutbound IPv4_out
    removeWwpsCoreOutbound IPv6_out
    removeWwpsCoreOutbound socks5_outbound
    removeWwpsCoreOutbound blackhole_out
    removeWwpsCoreOutbound wireguard_out_IPv6
    removeWwpsCoreOutbound wireguard_out_IPv4
    addXrayOutbound z_direct_outbound
...
# 初始化wwps-box配置文件
initWwpsBoxConfig() {
  echoContent skyBlue "\n进度 $2/${totalProgress} : 初始化wwps-box配置"
  ...
  if [[ -z ${addClientsStatus} ]]; then
    ...
    if [[ -n ${customUUID} ]]; then
      ...
      uuid=$(/etc/wwps/wwps-box/wwps-box generate uuid)
    fi
  ...
  if echo "${selectCustomInstallType}" | grep -q ",0," || [[ $1 == "all" ]]; then
    ...
    echo
    mapfile -t result < <(initWwpsBoxPort "${wwpsBoxVLESSVisionPort}")
    ...
    cat <<EOF >/etc/wwps/wwps-box/conf/config/02_VLESS_TCP_inbounds.json
{
    "inbounds":[
        {
          "type": "vless",
          ...
          "tag":"VLESSTCP",
          "users":$(initWwpsBoxClients 0),
          "tls":{
            "server_name": "${sslDomain}",
            "enabled": true,
            "certificate_path": "/etc/wwps/tls/${sslDomain}.crt",
            "key_path": "/etc/wwps/tls/${sslDomain}.key"
          }
        }
    ]
}
EOF
    ...
    rm /etc/wwps/wwps-box/conf/config/02_VLESS_TCP_inbounds.json >/dev/null 2>&1
  fi
  ...
  if echo "${selectCustomInstallType}" | grep -q ",1," || [[ $1 == "all" ]]; then
    ...
    echoContent skyBlue "\n开始配置VLESS+WS协议端口"
    echo
    mapfile -t result < <(initWwpsBoxPort "${wwpsBoxVLESSWSPort}")
    echoContent green "\n ---> VLESS_WS端口：${result[-1]}"
    ...
    randomPathFunction
    checkPortOpen "${result[-1]}" "${domain}"
    cat <<EOF >/etc/wwps/wwps-box/conf/config/03_VLESS_WS_inbounds.json
{
    "inbounds":[
        {
          "type": "vless",
...
EOF
    ...
    rm /etc/wwps/wwps-box/conf/config/03_VLESS_WS_inbounds.json >/dev/null 2>&1
  fi
  ...
  if echo "${selectCustomInstallType}" | grep -q ",3," || [[ $1 == "all" ]]; then
    ...
    echoContent skyBlue "\n开始配置VMess+ws协议端口"
    echo
    mapfile -t result < <(initWwpsBoxPort "${wwpsBoxVMessWSPort}")
    echoContent green "\n ---> VMess_ws端口：${result[-1]}"
    ...
    randomPathFunction
    checkPortOpen "${result[-1]}" "${domain}"
    cat <<EOF >/etc/wwps/wwps-box/conf/config/05_VMess_WS_inbounds.json
{
    "inbounds":[
        {
          "type": "vmess",
...
EOF
    ...
    rm /etc/wwps/wwps-box/conf/config/05_VMess_WS_inbounds.json >/dev/null 2>&1
  fi
  ...
  if echo "${selectCustomInstallType}" | grep -q ",7," || [[ $1 == "all" ]]; then
    ...
    echo
    mapfile -t result < <(initWwpsBoxPort "${wwpsBoxVLESSRealityVisionPort}")
    echoContent green "\n ---> VLESS_Reality_Vision端口：${result[-1]}"
    cat <<EOF >/etc/wwps/wwps-box/conf/config/07_VLESS_vision_reality_inbounds.json
{
  "inbounds": [
    {
      "type": "vless",
...
EOF
    ...
    rm /etc/wwps/wwps-box/conf/config/07_VLESS_vision_reality_inbounds.json >/dev/null 2>&1
  fi
  ...
  if echo "${selectCustomInstallType}" | grep -q ",8," || [[ $1 == "all" ]]; then
    ...
    echo
    mapfile -t result < <(initWwpsBoxPort "${wwpsBoxVLESSRealityGRPCPort}")
    echoContent green "\n ---> VLESS_Reality_gPRC端口：${result[-1]}"
    cat <<EOF >/etc/wwps/wwps-box/conf/config/08_VLESS_vision_gRPC_inbounds.json
{
  "inbounds": [
    {
      "type": "vless",
...
EOF
    ...
    rm /etc/wwps/wwps-box/conf/config/08_VLESS_vision_gRPC_inbounds.json >/dev/null 2>&1
  fi
  ...
  if echo "${selectCustomInstallType}" | grep -q ",6," || [[ $1 == "all" ]]; then
    ...
    echoContent skyBlue "\n开始配置Hysteria2协议端口"
    echo
    mapfile -t result < <(initWwpsBoxPort "${wwpsBoxHysteria2Port}")
    echoContent green "\n ---> Hysteria2端口：${result[-1]}"
    initHysteria2Network
    cat <<EOF >/etc/wwps/wwps-box/conf/config/06_hysteria2_inbounds.json
{
    "inbounds": [
        {
            "type": "hysteria2",
...
EOF
    ...
    rm /etc/wwps/wwps-box/conf/config/06_hysteria2_inbounds.json >/dev/null 2>&1
  fi
  ...
  if echo "${selectCustomInstallType}" | grep -q ",4," || [[ $1 == "all" ]]; then
    ...
    echoContent skyBlue "\n开始配置Trojan协议端口"
    echo
    mapfile -t result < <(initWwpsBoxPort "${wwpsBoxTrojanPort}")
    echoContent green "\n ---> Trojan端口：${result[-1]}"
    cat <<EOF >/etc/wwps/wwps-box/conf/config/04_trojan_TCP_inbounds.json
{
    "inbounds": [
        {
            "type": "trojan",
...
EOF
    ...
    rm /etc/wwps/wwps-box/conf/config/04_trojan_TCP_inbounds.json >/dev/null 2>&1
  fi
  ...
  if echo "${selectCustomInstallType}" | grep -q ",9," || [[ $1 == "all" ]]; then
    ...
    echoContent skyBlue "\n开始配置Tuic协议端口"
    echo
    mapfile -t result < <(initWwpsBoxPort "${wwpsBoxTuicPort}")
    echoContent green "\n ---> Tuic端口：${result[-1]}"
    initTuicProtocol
    cat <<EOF >/etc/wwps/wwps-box/conf/config/09_tuic_inbounds.json
{
     "inbounds": [
        {
            "type": "tuic",
...
EOF
    ...
    rm /etc/wwps/wwps-box/conf/config/09_tuic_inbounds.json >/dev/null 2>&1
  fi
  ...
  if echo "${selectCustomInstallType}" | grep -q ",10," || [[ $1 == "all" ]]; then
    ...
    echoContent skyBlue "\n开始配置Naive协议端口"
    echo
    mapfile -t result < <(initWwpsBoxPort "${wwpsBoxNaivePort}")
    echoContent green "\n ---> Naive端口：${result[-1]}"
    cat <<EOF >/etc/wwps/wwps-box/conf/config/10_naive_inbounds.json
{
     "inbounds": [
        {
            "type": "naive",
...
EOF
    ...
    rm /etc/wwps/wwps-box/conf/config/10_naive_inbounds.json >/dev/null 2>&1
  fi
  if echo "${selectCustomInstallType}" | grep -q ",11," || [[ $1 == "all" ]]; then
    echoContent yellow "\n===================== 配置VMess+HTTPUpgrade =====================\n"
    echoContent skyBlue "\n开始配置VMess+HTTPUpgrade协议端口"
    echo
    mapfile -t result < <(initWwpsBoxPort "${wwpsBoxVMessHTTPUpgradePort}")
    ...
    handleWwpsBox stop
    randomPathFunction
    rm -rf "${nginxConfigPath}wwps_box_VMess_HTTPUpgrade.conf" >/dev/null 2>&1
    checkPortOpen "${result[-1]}" "${domain}"
    wwpsBoxNginxConfig "$1" "${result[-1]}"
    bootStartup nginx
    cat <<EOF >/etc/wwps/wwps-box/conf/config/11_VMess_HTTPUpgrade_inbounds.json
{
    "inbounds":[
        {
          "type": "vmess",
...
EOF
    ...
    rm /etc/wwps/wwps-box/conf/config/11_VMess_HTTPUpgrade_inbounds.json >/dev/null 2>&1
  fi
  ...
  if echo "${selectCustomInstallType}" | grep -q ",13," || [[ $1 == "all" ]]; then
    ...
    echoContent skyBlue "\n开始配置AnyTLS协议端口"
    echo
    mapfile -t result < <(initWwpsBoxPort "${wwpsBoxAnyTLSPort}")
    echoContent green "\n ---> AnyTLS端口：${result[-1]}"
    cat <<EOF >/etc/wwps/wwps-box/conf/config/13_anytls_inbounds.json
{
    "inbounds": [
        {
            "type": "anytls",
...
EOF
    ...
    rm /etc/wwps/wwps-box/conf/config/13_anytls_inbounds.json >/dev/null 2>&1
  fi

  if [[ -z $3 ]]; then
    removeWwpsBoxConfig wireguard_endpoints_IPv4_route
    removeWwpsBoxConfig wireguard_endpoints_IPv6_route
    removeWwpsBoxConfig wireguard_endpoints_IPv4
    removeWwpsBoxConfig wireguard_endpoints_IPv6

    removeWwpsBoxConfig IPv4_out
    removeWwpsBoxConfig IPv6_out
    removeWwpsBoxConfig IPv6_route
    removeWwpsBoxConfig block
    removeWwpsBoxConfig cn_block_outbound
    removeWwpsBoxConfig cn_block_route
    removeWwpsBoxConfig 01_direct_outbound
    removeWwpsBoxConfig socks5_outbound.json
    removeWwpsBoxConfig block_domain_outbound
    removeWwpsBoxConfig dns
  fi
}
# 初始化 wwps-box订阅配置
initSubscribeLocalConfig() {
  rm -rf /etc/wwps/subscribe_local/wwps-box/*
}
# 通用
defaultBase64Code() {
  local type=$1
  ...
  if [[ ! -f "/etc/wwps/subscribe_local/wwps-box/${user}" ]]; then
    echo [] >"/etc/wwps/subscribe_local/wwps-box/${user}"
  fi
  local wwpsBoxSubscribeLocalConfig=
  if [[ ${type} == "vlesstcp" ]]; then
    ...
    cat <<EOF >>"/etc/wwps/subscribe_local/default/${user}"
...
EOF
    cat <<EOF >>"/etc/wwps/subscribe_local/clashMeta/${user}"
  - name: "${email}"
    type: vless
    server: ${currentHost}
    port: ${port}
...
EOF
    wwpsBoxSubscribeLocalConfig=$(jq -r ". += [{\"tag\":\"${email}\",\"type\":\"vless\",\"server\":\"${currentHost}\",\"server_port\":${port},\"uuid\":\"${id}\",\"flow\":\"xtls-rprx-vision\",\"tls\":{\"enabled\":true,\"server_name\":\"${currentHost}\",\"utls\":{\"enabled\":true,\"fingerprint\":\"chrome\"}},\"packet_encoding\":\"xudp\"}]" "/etc/wwps/subscribe_local/wwps-box/${user}")
    echo "${wwpsBoxSubscribeLocalConfig}" | jq . >"/etc/wwps/subscribe_local/wwps-box/${user}"
  ...
  - name: "${email}"
    ...
    ws-opts:
      ...
      headers:
        Host: ${currentHost}
EOF
    wwpsBoxSubscribeLocalConfig=$(jq -r ". += [{\"tag\":\"${email}\",\"type\":\"vmess\",\"server\":\"${add}\",\"server_port\":${port},\"uuid\":\"${id}\",\"alter_id\":0,\"tls\":{\"enabled\":true,\"server_name\":\"${currentHost}\",\"utls\":{\"enabled\":true,\"fingerprint\":\"chrome\"}},\"packet_encoding\":\"packetaddr\",\"transport\":{\"type\":\"ws\",\"path\":\"${path}\",\"max_early_data\":2048,\"early_data_header_name\":\"Sec-WebSocket-Protocol\"}}]" "/etc/wwps/subscribe_local/wwps-box/${user}")
...
EOF
    ...
    wwpsBoxSubscribeLocalConfig=$(jq -r ". += [{\"tag\":\"${email}\",\"type\":\"vless\",\"server\":\"${add}\",\"server_port\":${port},\"uuid\":\"${id}\",\"tls\":{\"enabled\":true,\"server_name\":\"${currentHost}\",\"utls\":{\"enabled\":true,\"fingerprint\":\"chrome\"}},\"multiplex\":{\"enabled\":false,\"protocol\":\"smux\",\"max_streams\":32},\"packet_encoding\":\"xudp\",\"transport\":{\"type\":\"ws\",\"path\":\"${path}\",\"headers\":{\"Host\":\"${currentHost}\"}}}]" "/etc/wwps/subscribe_local/wwps-box/${user}")
    echo "${wwpsBoxSubscribeLocalConfig}" | jq . >"/etc/wwps/subscribe_local/wwps-box/${user}"
    ...
    echoContent green "    vless://${id}@${add}:${port}?encryption=none&security=reality&type=xhttp&sni=${wwpsCoreVLESSRealityXHTTPServerName}&host=${wwpsCoreVLESSRealityXHTTPServerName}&fp=chrome&path=${path}&pbk=${currentRealityXHTTPPublicKey}&sid=6ba85179e30d4fc2#${email}\n"

    echoContent yellow " ---> 格式化明文(VLESS+reality+XHTTP)"
    echoContent green "协议类型:VLESS reality，地址:${add}，publicKey:${currentRealityXHTTPPublicKey}，shortId: 6ba85179e30d4fc2,serverNames：${wwpsCoreVLESSRealityXHTTPServerName}，端口:${port}，路径：${path}，SNI:${wwpsCoreVLESSRealityXHTTPServerName}，伪装域名:${wwpsCoreVLESSRealityXHTTPServerName}，用户ID:${id}，传输方式:xhttp，账户名:${email}\n"
    cat <<EOF >>"/etc/wwps/subscribe_local/default/${user}"
vless://${id}@${add}:${port}?encryption=none&security=reality&type=xhttp&sni=${wwpsCoreVLESSRealityXHTTPServerName}&fp=chrome&path=${path}&pbk=${currentRealityXHTTPPublicKey}&sid=6ba85179e30d4fc2#${email}
EOF
    echoContent yellow " ---> 二维码 VLESS(VLESS+reality+XHTTP)"
    echoContent green "    https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=vless%3A%2F%2F${id}%40${add}%3A${port}%3Fencryption%3Dnone%26security%3Dreality%26type%3Dxhttp%26sni%3D${wwpsCoreVLESSRealityXHTTPServerName}%26fp%3Dchrome%26path%3D${path}%26host%3D${wwpsCoreVLESSRealityXHTTPServerName}%26pbk%3D${currentRealityXHTTPPublicKey}%26sid%3D6ba85179e30d4fc2%23${email}\n"
...
EOF
    ...
    wwpsBoxSubscribeLocalConfig=$(jq -r ". += [{\"tag\":\"${email}\",\"type\": \"vless\",\"server\": \"${add}\",\"server_port\": ${port},\"uuid\": \"${id}\",\"tls\": {  \"enabled\": true,  \"server_name\": \"${currentHost}\",  \"utls\": {    \"enabled\": true,    \"fingerprint\": \"chrome\"  }},\"packet_encoding\": \"xudp\",\"transport\": {  \"type\": \"grpc\",  \"service_name\": \"${currentPath}grpc\"}}]" "/etc/wwps/subscribe_local/wwps-box/${user}")
    echo "${wwpsBoxSubscribeLocalConfig}" | jq . >"/etc/wwps/subscribe_local/wwps-box/${user}"
...
EOF
  ...
  - name: "${email}"
    ...
    sni: ${currentHost}
EOF
    wwpsBoxSubscribeLocalConfig=$(jq -r ". += [{\"tag\":\"${email}\",\"type\":\"trojan\",\"server\":\"${currentHost}\",\"server_port\":${port},\"password\":\"${id}\",\"tls\":{\"alpn\":[\"http/1.1\"],\"enabled\":true,\"server_name\":\"${currentHost}\",\"utls\":{\"enabled\":true,\"fingerprint\":\"chrome\"}}}]" "/etc/wwps/subscribe_local/wwps-box/${user}")
    echo "${wwpsBoxSubscribeLocalConfig}" | jq . >"/etc/wwps/subscribe_local/wwps-box/${user}"
...
EOF
    ...
    wwpsBoxSubscribeLocalConfig=$(jq -r ". += [{\"tag\":\"${email}\",\"type\":\"trojan\",\"server\":\"${add}\",\"server_port\":${port},\"password\":\"${id}\",\"tls\":{\"enabled\":true,\"server_name\":\"${currentHost}\",\"insecure\":true,\"utls\":{\"enabled\":true,\"fingerprint\":\"chrome\"}},\"transport\":{\"type\":\"grpc\",\"service_name\":\"${currentPath}trojangrpc\",\"idle_timeout\":\"15s\",\"ping_timeout\":\"15s\",\"permit_without_stream\":false},\"multiplex\":{\"enabled\":false,\"protocol\":\"smux\",\"max_streams\":32}}]" "/etc/wwps/subscribe_local/wwps-box/${user}")
    echo "${wwpsBoxSubscribeLocalConfig}" | jq . >"/etc/wwps/subscribe_local/wwps-box/${user}"
    ...
    echoContent green "    hysteria2://${id}@${currentHost}:${wwpsBoxHysteria2Port}?${multiPort}peer=${currentHost}&insecure=0&sni=${currentHost}&alpn=h3#${email}\n"
    cat <<EOF >>"/etc/wwps/subscribe_local/default/${user}"
hysteria2://${id}@${currentHost}:${wwpsBoxHysteria2Port}?${multiPort}peer=${currentHost}&insecure=0&sni=${currentHost}&alpn=h3#${email}
EOF
...
EOF
    ...
    wwpsBoxSubscribeLocalConfig=$(jq -r ". += [{\"tag\":\"${email}\",\"type\":\"hysteria2\",\"server\":\"${currentHost}\",\"server_port\":${wwpsBoxHysteria2Port},\"up_mbps\":${hysteria2ClientUploadSpeed},\"down_mbps\":${hysteria2ClientDownloadSpeed},\"password\":\"${id}\",\"tls\":{\"enabled\":true,\"server_name\":\"${currentHost}\",\"alpn\":[\"h3\"]}}]" "/etc/wwps/subscribe_local/wwps-box/${user}")
    echo "${wwpsBoxSubscribeLocalConfig}" | jq . >"/etc/wwps/subscribe_local/wwps-box/${user}"

    echoContent yellow " ---> 二维码 Hysteria2(TLS)"
    echoContent green "    https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=hysteria2%3A%2F%2F${id}%40${currentHost}%3A${wwpsBoxHysteria2Port}%3F${multiPortEncode}peer%3D${currentHost}%26insecure%3D0%26sni%3D${currentHost}%26alpn%3Dh3%23${email}\n"

  elif [[ ${type} == "vlessReality" ]]; then
    local realityServerName=${wwpsCoreVLESSRealityServerName}
    local publicKey=${currentRealityPublicKey}
    local realityMldsa65Verify=${currentRealityMldsa65Verify}

    if [[ ${coreInstallType} == "2" ]]; then
      realityServerName=${wwpsBoxVLESSRealityVisionServerName}
      publicKey=${wwpsBoxVLESSRealityPublicKey}
    fi
...
EOF
    ...
    wwpsBoxSubscribeLocalConfig=$(jq -r ". += [{\"tag\":\"${email}\",\"type\":\"vless\",\"server\":\"$(getPublicIP)\",\"server_port\":${port},\"uuid\":\"${id}\",\"flow\":\"xtls-rprx-vision\",\"tls\":{\"enabled\":true,\"server_name\":\"${realityServerName}\",\"utls\":{\"enabled\":true,\"fingerprint\":\"chrome\"},\"reality\":{\"enabled\":true,\"public_key\":\"${publicKey}\",\"short_id\":\"6ba85179e30d4fc2\"}},\"packet_encoding\":\"xudp\"}]" "/etc/wwps/subscribe_local/wwps-box/${user}")
    echo "${wwpsBoxSubscribeLocalConfig}" | jq . >"/etc/wwps/subscribe_local/wwps-box/${user}"
    ...
    if [[ ${coreInstallType} == "2" ]]; then
      realityServerName=${wwpsBoxVLESSRealityGRPCServerName}
      publicKey=${wwpsBoxVLESSRealityPublicKey}
    fi
...
EOF
    ...
    wwpsBoxSubscribeLocalConfig=$(jq -r ". += [{\"tag\":\"${email}\",\"type\": \"tuic\",\"server\": \"${currentHost}\",\"server_port\": ${port},\"uuid\": \"${tuicUUID}\",\"password\": \"${tuicPassword}\",\"congestion_control\": \"${tuicAlgorithm}\",\"tls\": {\"enabled\": true,\"server_name\": \"${currentHost}\",\"alpn\": [\"h3\"]}}]" "/etc/wwps/subscribe_local/wwps-box/${user}")
    echo "${wwpsBoxSubscribeLocalConfig}" | jq . >"/etc/wwps/subscribe_local/wwps-box/${user}"
...
EOF
    wwpsBoxSubscribeLocalConfig=$(jq -r ". += [{\"tag\":\"${email}\",\"type\":\"vmess\",\"server\":\"${add}\",\"server_port\":${port},\"uuid\":\"${id}\",\"security\":\"auto\",\"alter_id\":0,\"tls\":{\"enabled\":true,\"server_name\":\"${currentHost}\",\"utls\":{\"enabled\":true,\"fingerprint\":\"chrome\"}},\"packet_encoding\":\"packetaddr\",\"transport\":{\"type\":\"httpupgrade\",\"path\":\"${path}\"}}]" "/etc/wwps/subscribe_local/wwps-box/${user}")
    ...
    echoContent green "协议类型:anytls，地址:${currentHost}，端口:${wwpsBoxAnyTLSPort}，用户ID:${id}，传输方式:tcp，账户名:${email}\n"

    echoContent green "    anytls://${id}@${currentHost}:${wwpsBoxAnyTLSPort}?peer=${currentHost}&insecure=0&sni=${currentHost}#${email}\n"
    cat <<EOF >>"/etc/wwps/subscribe_local/default/${user}"
anytls://${id}@${currentHost}:${wwpsBoxAnyTLSPort}?peer=${currentHost}&insecure=0&sni=${currentHost}#${email}
EOF
    cat <<EOF >>"/etc/wwps/subscribe_local/clashMeta/${user}"
  - name: "${email}"
    type: anytls
    port: ${wwpsBoxAnyTLSPort}
    server: ${currentHost}
    password: ${id}
    client-fingerprint: chrome
    udp: true
...
EOF
    ...
    wwpsBoxSubscribeLocalConfig=$(jq -r ". += [{\"tag\":\"${email}\",\"type\":\"anytls\",\"server\":\"${currentHost}\",\"server_port\":${wwpsBoxAnyTLSPort},\"password\":\"${id}\",\"tls\":{\"enabled\":true,\"server_name\":\"${currentHost}\"}}]" "/etc/wwps/subscribe_local/wwps-box/${user}")
    echo "${wwpsBoxSubscribeLocalConfig}" | jq . >"/etc/wwps/subscribe_local/wwps-box/${user}"

    echoContent yellow " ---> 二维码 AnyTLS"
    echoContent green "    https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=anytls%3A%2F%2F${id}%40${currentHost}%3A${wwpsBoxAnyTLSPort}%3Fpeer%3D${currentHost}%26insecure%3D0%26sni%3D${currentHost}%23${email}\n"
  fi
...
showAccounts() {
  ...
  if echo ${currentInstallProtocolType} | grep -q ",0,"; then
    ...
    jq .inbounds[0].settings.clients//.inbounds[0].users ${configPath}02_VLESS_TCP_inbounds.json | jq -c '.[]' | while read -r user; do
      ...
      echoContent skyBlue "\n ---> 账号:${email}"
      echo
      defaultBase64Code vlesstcp "${currentDefaultPort}${wwpsBoxVLESSVisionPort}" "${email}" "$(echo "${user}" | jq -r .id//.uuid)"
    done
  fi
  ...
  if echo ${currentInstallProtocolType} | grep -q ",1,"; then
    ...
    jq .inbounds[0].settings.clients//.inbounds[0].users ${configPath}03_VLESS_WS_inbounds.json | jq -c '.[]' | while read -r user; do
      ...
      if [[ ${coreInstallType} == "2" ]]; then
        vlessWSPort="${wwpsBoxVLESSWSPort}"
      fi
      echo
  ...
  if echo ${currentInstallProtocolType} | grep -q ",4,"; then
    ...
    jq .inbounds[0].settings.clients//.inbounds[0].users ${configPath}04_trojan_TCP_inbounds.json | jq -c '.[]' | while read -r user; do
      ...
      defaultBase64Code trojan "${currentDefaultPort}${wwpsBoxTrojanPort}" "${email}" "$(echo "${user}" | jq -r .password)"
    done
  fi
  # VLESS grpc
  if echo ${currentInstallProtocolType} | grep -q ",5,"; then
  ...
  if echo ${currentInstallProtocolType} | grep -q ",6," || [[ -n ${hysteriaPort} ]]; then
    readPortHopping "hysteria2" "${wwpsBoxHysteria2Port}"
    echoContent skyBlue "\n================================  Hysteria2 TLS [推荐] ================================\n"
    local path="${configPath}"
    if [[ ${coreInstallType} == "1" ]]; then
      path="${wwpsBoxConfigPath}"
    fi
    ...
    if [[ -n ${hysteria2PortHoppingStart} && -n ${hysteria2PortHoppingEnd} ]]; then
      ...
      hysteria2DefaultPort=${wwpsBoxHysteria2Port}
    fi
  ...
  if echo ${currentInstallProtocolType} | grep -q ",7,"; then
    ...
    jq .inbounds[0].settings.clients//.inbounds[0].users ${configPath}07_VLESS_vision_reality_inbounds.json | jq -c '.[]' | while read -r user; do
      ...
      echoContent skyBlue "\n ---> 账号:${email}"
      echo
      defaultBase64Code vlessReality "${wwpsCoreVLESSRealityVisionPort}${wwpsBoxVLESSRealityVisionPort}" "${email}" "$(echo "${user}" | jq -r .id//.uuid)"
    done
  fi
  # VLESS reality gRPC
  if echo ${currentInstallProtocolType} | grep -q ",8,"; then
    ...
    jq .inbounds[0].settings.clients//.inbounds[0].users ${configPath}08_VLESS_vision_gRPC_inbounds.json | jq -c '.[]' | while read -r user; do
      ...
      echoContent skyBlue "\n ---> 账号:${email}"
      echo
      defaultBase64Code vlessRealityGRPC "${wwpsCoreVLESSRealityVisionPort}${wwpsBoxVLESSRealityGRPCPort}" "${email}" "$(echo "${user}" | jq -r .id//.uuid)"
    done
  fi
  # tuic
  if echo ${currentInstallProtocolType} | grep -q ",9," || [[ -n ${tuicPort} ]]; then
    ...
    jq -r -c '.inbounds[].users[]' "${path}09_tuic_inbounds.json" | while read -r user; do
      echoContent skyBlue "\n ---> 账号:$(echo "${user}" | jq -r .name)"
      echo
      defaultBase64Code tuic "${wwpsBoxTuicPort}" "$(echo "${user}" | jq -r .name)" "$(echo "${user}" | jq -r .uuid)_$(echo "${user}" | jq -r .password)"
    done

  fi
  # naive
  if echo ${currentInstallProtocolType} | grep -q ",10," || [[ -n ${wwpsBoxNaivePort} ]]; then
    ...
    jq -r -c '.inbounds[]|.users[]' "${configPath}10_naive_inbounds.json" | while read -r user; do
      ...
      echo
      defaultBase64Code naive "${wwpsBoxNaivePort}" "$(echo "${user}" | jq -r .username)" "$(echo "${user}" | jq -r .password)"
    done
  ...
  if echo ${currentInstallProtocolType} | grep -q ",11,"; then
    ...
    if [[ ${coreInstallType} == "1" ]]; then
      ...
      path="${wwpsBoxVMessHTTPUpgradePath}"
    fi
    jq .inbounds[0].settings.clients//.inbounds[0].users ${configPath}11_VMess_HTTPUpgrade_inbounds.json | jq -c '.[]' | while read -r user; do
      ...
      if [[ ${coreInstallType} == "2" ]]; then
        vmessHTTPUpgradePort="${wwpsBoxVMessHTTPUpgradePort}"
      fi
  ...
  if echo ${currentInstallProtocolType} | grep -q ",12,"; then
    ...
    jq .inbounds[0].settings.clients//.inbounds[0].users ${configPath}12_VLESS_XHTTP_inbounds.json | jq -c '.[]' | while read -r user; do
      ...
      while read -r line; do
        echoContent skyBlue "\n ---> 账号:${email}${count}"
        if [[ -n ${line} ]]; then
          defaultBase64Code vlessXHTTP "${wwpsCoreVLESSRealityXHTTPort}" "${email}${count}" "$(echo "${user}" | jq -r .id//.uuid)" "${line}" "${path}"
          count=$((count + 1))
          echo
        fi
      done < <(echo "${currentCDNAddress}" | tr ',' '\n')
  ...
  if echo ${currentInstallProtocolType} | grep -q ",13,"; then
    ...
    jq -r -c '.inbounds[]|.users[]' "${configPath}13_anytls_inbounds.json" | while read -r user; do
      echoContent skyBlue "\n ---> 账号:$(echo "${user}" | jq -r .name)"
      echo
      defaultBase64Code anytls "${wwpsBoxAnyTLSPort}" "$(echo "${user}" | jq -r .name)" "$(echo "${user}" | jq -r .password)"
    done
...
backupNginxConfig() {
  if [[ $1 == "backup" ]]; then
    cp ${nginxConfigPath}alone.conf /etc/wwps/alone_backup.conf
    echoContent green " ---> nginx配置文件备份成功"
  fi

  if [[ $1 == "restoreBackup" ]] && [[ -f "/etc/wwps/alone_backup.conf" ]]; then
    cp /etc/wwps/alone_backup.conf ${nginxConfigPath}alone.conf
    echoContent green " ---> nginx配置文件恢复备份成功"
    rm /etc/wwps/alone_backup.conf
  fi
...
updateNginxBlog() {
  ...
  if ! echo "${currentInstallProtocolType}" | grep -q ",0," || [[ -z ${coreInstallType} ]]; then
    echoContent red "\n ---> 由于环境依赖，请先安装wwps-core的VLESS_TCP_TLS_Vision"
    exit 0
  fi
  echoContent red "=============================================================="
  ...
  if [[ ${selectInstallNginxBlogType} =~ ^[1-9]$ ]]; then
    ...
    if [[ ${release} == "alpine" ]]; then
      wget -q -P "${nginxStaticPath}" "https://raw.githubusercontent.com/youugiuhiuh/Wuthering_Waves_Private_Server/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip"
    else
      wget -q "${wgetShowProgressStatus}" -P "${nginxStaticPath}" "https://raw.githubusercontent.com/youugiuhiuh/Wuthering_Waves_Private_Server/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip"
    fi
...
unInstall() {
  ...
  if [[ ${release} == "alpine" ]]; then
    ...
    if [[ ${coreInstallType} == "2" || -n ${wwpsBoxConfigPath} ]]; then
      handleWwpsBox stop
      rc-update del wwps-box default
      rm -rf /etc/init.d/wwps-box
      echoContent green " ---> 删除wwps-box开机自启完成"
    fi
  else
    if [[ ${coreInstallType} == "1" ]]; then
      handleWwpsCore stop
      rm -rf /etc/systemd/system/wwps-core.service
      echoContent green " ---> 删除wwps-core开机自启完成"
    fi
    if [[ ${coreInstallType} == "2" || -n ${wwpsBoxConfigPath} ]]; then
      handleWwpsBox stop
      rm -rf /etc/systemd/system/wwps-box.service
...
manageCDN() {
  ...
  if echo "${currentInstallProtocolType}" | grep -qE ",1,|,2,|,3,|,5,|,11,"; then
    ...
    if [[ -n ${setCDNDomain} ]]; then
      echo >/etc/wwps/cdn
      echo "${setCDNDomain}" >"/etc/wwps/cdn"
...
addUser() {
  ...
  while [[ ${userNum} -gt 0 ]]; do
    ...
    if echo "${currentInstallProtocolType}" | grep -q ",0,"; then
      local clients=
      if [[ ${coreInstallType} == "1" ]]; then
        clients=$(initWwpsCoreClients 0 "${uuid}" "${email}")
      elif [[ ${coreInstallType} == "2" ]]; then
        clients=$(initWwpsBoxClients 0 "${uuid}" "${email}")
      fi
    ...
    if echo "${currentInstallProtocolType}" | grep -q ",1,"; then
      local clients=
      if [[ ${coreInstallType} == "1" ]]; then
        clients=$(initWwpsCoreClients 1 "${uuid}" "${email}")
      elif [[ ${coreInstallType} == "2" ]]; then
        clients=$(initWwpsBoxClients 1 "${uuid}" "${email}")
      fi
    ...
    if echo "${currentInstallProtocolType}" | grep -q ",2,"; then
      local clients=
      if [[ ${coreInstallType} == "1" ]]; then
        clients=$(initWwpsCoreClients 2 "${uuid}" "${email}")
      elif [[ ${coreInstallType} == "2" ]]; then
        clients=$(initWwpsBoxClients 2 "${uuid}" "${email}")
      fi
    ...
    if echo "${currentInstallProtocolType}" | grep -q ",3,"; then
      local clients=
      if [[ ${coreInstallType} == "1" ]]; then
        clients=$(initWwpsCoreClients 3 "${uuid}" "${email}")
      elif [[ ${coreInstallType} == "2" ]]; then
        clients=$(initWwpsBoxClients 3 "${uuid}" "${email}")
      fi
    ...
    if echo "${currentInstallProtocolType}" | grep -q ",4,"; then
      local clients=
      if [[ ${coreInstallType} == "1" ]]; then
        clients=$(initWwpsCoreClients 4 "${uuid}" "${email}")
      elif [[ ${coreInstallType} == "2" ]]; then
        clients=$(initWwpsBoxClients 4 "${uuid}" "${email}")
      fi
    ...
    if echo "${currentInstallProtocolType}" | grep -q ",5,"; then
      local clients=
      if [[ ${coreInstallType} == "1" ]]; then
        clients=$(initWwpsCoreClients 5 "${uuid}" "${email}")
      elif [[ ${coreInstallType} == "2" ]]; then
        clients=$(initWwpsBoxClients 5 "${uuid}" "${email}")
      fi
    ...
    if echo "${currentInstallProtocolType}" | grep -q ",7,"; then
      ...
      if [[ ${coreInstallType} == "1" ]]; then
        clients=$(initWwpsCoreClients 7 "${uuid}" "${email}")
        realityUserConfig=".inbounds[1].settings.clients"
      elif [[ ${coreInstallType} == "2" ]]; then
        clients=$(initWwpsBoxClients 7 "${uuid}" "${email}")
    ...
    if echo "${currentInstallProtocolType}" | grep -q ",8,"; then
      local clients=
      if [[ ${coreInstallType} == "1" ]]; then
        clients=$(initWwpsCoreClients 8 "${uuid}" "${email}")
      elif [[ ${coreInstallType} == "2" ]]; then
        clients=$(initWwpsBoxClients 8 "${uuid}" "${email}")
      fi
    ...
    if echo ${currentInstallProtocolType} | grep -q ",6,"; then
      ...
      if [[ ${coreInstallType} == "1" ]]; then
        clients=$(initWwpsCoreClients 6 "${uuid}" "${email}")
      elif [[ -n ${wwpsBoxConfigPath} ]]; then
        clients=$(initWwpsBoxClients 6 "${uuid}" "${email}")
      fi

      clients=$(jq -r ".inbounds[0].users = ${clients}" "${wwpsBoxConfigPath}06_hysteria2_inbounds.json")
      echo "${clients}" | jq . >"${wwpsBoxConfigPath}06_hysteria2_inbounds.json"
    fi
    ...
    if echo ${currentInstallProtocolType} | grep -q ",9,"; then
      local clients=
      if [[ ${coreInstallType} == "1" ]]; then
        clients=$(initWwpsCoreClients 9 "${uuid}" "${email}")
      elif [[ ${coreInstallType} == "2" ]]; then
        clients=$(initWwpsBoxClients 9 "${uuid}" "${email}")
      fi

      clients=$(jq -r ".inbounds[0].users = ${clients}" "${wwpsBoxConfigPath}09_tuic_inbounds.json")

      echo "${clients}" | jq . >"${wwpsBoxConfigPath}09_tuic_inbounds.json"
    fi
    # naive
    if echo ${currentInstallProtocolType} | grep -q ",10,"; then
      local clients=
      clients=$(initWwpsBoxClients 10 "${uuid}" "${email}")
      clients=$(jq -r ".inbounds[0].users = ${clients}" "${wwpsBoxConfigPath}10_naive_inbounds.json")

      echo "${clients}" | jq . >"${wwpsBoxConfigPath}10_naive_inbounds.json"
    fi
    # VMess WS
    if echo "${currentInstallProtocolType}" | grep -q ",11,"; then
      local clients=
      if [[ ${coreInstallType} == "1" ]]; then
        clients=$(initWwpsCoreClients 11 "${uuid}" "${email}")
      elif [[ ${coreInstallType} == "2" ]]; then
        clients=$(initWwpsBoxClients 11 "${uuid}" "${email}")
      fi
    ...
    if echo "${currentInstallProtocolType}" | grep -q ",13,"; then
      local clients=
      clients=$(initWwpsBoxClients 13 "${uuid}" "${email}")
...
removeUser() {
  ...
  if [[ -n ${delUserIndex} ]]; then
    ...
    if echo ${currentInstallProtocolType} | grep -q ",6,"; then
      ...
      hysteriaResult=$(jq -r 'del(.inbounds[0].users['"${delUserIndex}"'])' "${wwpsBoxConfigPath}06_hysteria2_inbounds.json")
      echo "${hysteriaResult}" | jq . >"${wwpsBoxConfigPath}06_hysteria2_inbounds.json"
    fi
    if echo ${currentInstallProtocolType} | grep -q ",7,"; then
    ...
    if echo ${currentInstallProtocolType} | grep -q ",9,"; then
      local tuicResult
      tuicResult=$(jq -r 'del(.inbounds[0].users['"${delUserIndex}"'])' "${wwpsBoxConfigPath}09_tuic_inbounds.json")
      echo "${tuicResult}" | jq . >"${wwpsBoxConfigPath}09_tuic_inbounds.json"
    fi
    if echo ${currentInstallProtocolType} | grep -q ",10,"; then
      local naiveResult
      naiveResult=$(jq -r 'del(.inbounds[0].users['"${delUserIndex}"'])' "${wwpsBoxConfigPath}10_naive_inbounds.json")
      echo "${naiveResult}" | jq . >"${wwpsBoxConfigPath}10_naive_inbounds.json"
    fi
    # VMess HTTPUpgrade
    if echo ${currentInstallProtocolType} | grep -q ",11,"; then
      local vmessHTTPUpgradeResult
      vmessHTTPUpgradeResult=$(jq -r 'del(.inbounds[0].users['"${delUserIndex}"'])' "${wwpsBoxConfigPath}11_VMess_HTTPUpgrade_inbounds.json")
      echo "${vmessHTTPUpgradeResult}" | jq . >"${wwpsBoxConfigPath}11_VMess_HTTPUpgrade_inbounds.json"
      echo "${vmessHTTPUpgradeResult}" | jq . >${configPath}11_VMess_HTTPUpgrade_inbounds.json
    fi
    # AnyTLS
    if echo ${currentInstallProtocolType} | grep -q ",13,"; then
      local anyTLSResult
      anyTLSResult=$(jq -r 'del(.inbounds[0].users['"${delUserIndex}"'])' "${wwpsBoxConfigPath}13_anytls_inbounds.json")
      echo "${anyTLSResult}" | jq . >"${wwpsBoxConfigPath}13_anytls_inbounds.json"
    fi
    reloadCore
    readNginxSubscribe
    if [[ -n ${subscribePort} ]]; then
...
# 更新脚本
updateV2RayAgent() {
  echoContent skyBlue "\n进度  $1/${totalProgress} : 更新wwps脚本"
  rm -rf /etc/wwps/install.sh
  if [[ ${release} == "alpine" ]]; then
    wget -c -q -P /etc/wwps/ -N --no-check-certificate "https://raw.githubusercontent.com/SKIPPINGpetticoatconvent/v2ray-agent/master/install.sh"
  else
    wget -c -q "${wgetShowProgressStatus}" -P /etc/wwps/ -N --no-check-certificate "https://raw.githubusercontent.com/SKIPPINGpetticoatconvent/v2ray-agent/master/install.sh"
  fi

  sudo chmod 700 /etc/wwps/install.sh
  local version
  version=$(grep '当前版本：v' "/etc/wwps/install.sh" | awk -F "[v]" '{print $2}' | tail -n +2 | head -n 1 | awk -F '["]' '{print $1}')

  echoContent green "\n ---> 更新完毕"
  echoContent yellow " ---> 请手动执行[WW]打开脚本"
  echoContent green " ---> 当前版本：${version}\n"
  echoContent yellow "如更新不成功，请手动执行下面命令\n"
  echoContent skyBlue "wget -P /root -N --no-check-certificate https://raw.githubusercontent.com/youugiuhiuh/Wuthering_Waves_Private_Server/master/install.sh && chmod 700 /root/install.sh && /root/install.sh"
  echo
  exit 0
}
...
EOF
  ...
  3)
    ...
    ;;
  4)
    if [[ ! -f "/etc/wwps/crontab_tls.log" ]]; then
      touch /etc/wwps/crontab_tls.log
    fi
    tail -n 100 /etc/wwps/crontab_tls.log
    ;;
  5)
    tail -n 100 /etc/wwps/tls/acme.log
    ;;
...
aliasInstall() {
  ...
  if [[ -f "$HOME/install.sh" ]] && [[ -d "/etc/wwps" ]] && grep <"$HOME/install.sh" -q "作者:youugiuhiuh"; then
    mv "$HOME/install.sh" /etc/wwps/install.sh
    local vasmaType=
    if [[ -d "/usr/bin/" ]]; then
      if [[ ! -f "/usr/bin/WW" ]]; then
        ln -s /etc/wwps/install.sh /usr/bin/WW
        chmod 700 /usr/bin/WW
        vasmaType=true
      fi

      rm -rf "$HOME/install.sh"
    elif [[ -d "/usr/sbin" ]]; then
      if [[ ! -f "/usr/sbin/WW" ]]; then
        ln -s /etc/wwps/install.sh /usr/sbin/WW
        chmod 700 /usr/sbin/WW
        vasmaType=true
      fi
      rm -rf "$HOME/install.sh"
    fi
    if [[ ${vasmaType} == "true" ]]; then
      echoContent green "快捷方式创建成功，可执行[WW]重新打开脚本"
    fi
  fi
}
...
ipv6Routing() {
  ...
  if [[ ${ipv6Status} == "1" ]]; then
    ...
    if [[ -n ${wwpsBoxConfigPath} ]]; then
      addWwpsBoxRouteRule "IPv6_out" "${domainList}" "IPv6_route"
      addWwpsBoxOutbound 01_direct_outbound
      addWwpsBoxOutbound IPv6_out
      addWwpsBoxOutbound IPv4_out
    fi
    ...
    if [[ ${IPv6OutStatus} == "y" ]]; then
      if [[ ${coreInstallType} == "1" ]]; then
        addXrayOutbound IPv6_out
        removeWwpsCoreOutbound IPv4_out
        removeWwpsCoreOutbound z_direct_outbound
        removeWwpsCoreOutbound blackhole_out
        removeWwpsCoreOutbound wireguard_out_IPv4
        removeWwpsCoreOutbound wireguard_out_IPv6
        removeWwpsCoreOutbound socks5_outbound
      ...
      if [[ -n ${wwpsBoxConfigPath} ]]; then
        ...
        removeWwpsBoxConfig socks5_02_inbound_route
...
showIPv6Routing() {
  ...
  if [[ -n ${wwpsBoxConfigPath} ]]; then
    if [[ -f "${wwpsBoxConfigPath}IPv6_route.json" ]]; then
      echoContent yellow "wwps-box"
      jq -r -c '.route.rules[]|select (.outbound=="IPv6_out")' "${wwpsBoxConfigPath}IPv6_route.json" | jq -r
    elif [[ ! -f "${wwpsBoxConfigPath}IPv6_route.json" && -f "${wwpsBoxConfigPath}IPv6_out.json" ]]; then
      echoContent yellow "wwps-box"
      echoContent green " ---> 已设置IPv6全局分流"
    else
      echoContent yellow " ---> 未安装IPv6分流"
...
blacklist() {
  ...
  if [[ ${blacklistStatus} == "1" ]]; then
    ...
    if [[ -n ${wwpsBoxConfigPath} ]]; then
      addWwpsBoxRouteRule "block_domain_outbound" "${domainList}" "block_domain_route"
      addWwpsBoxOutbound "block_domain_outbound"
      addWwpsBoxOutbound "01_direct_outbound"
    fi
    echoContent green " ---> 添加完毕"
    ...
    if [[ -n ${wwpsBoxConfigPath} ]]; then
      ...
      addWwpsBoxRouteRule "cn_block_outbound" "cn" "cn_block_route"

      addWwpsBoxRouteRule "01_direct_outbound" "googleapis.com,googleapis.cn,xn--ngstr-lra8j.com,gstatic.com" "cn_01_google_play_route"

      addWwpsBoxOutbound "cn_block_outbound"
      addWwpsBoxOutbound "01_direct_outbound"
    fi
    ...
    if [[ -n ${wwpsBoxConfigPath} ]]; then
      ...
      removeWwpsBoxConfig "cn_01_google_play_route"

      removeWwpsBoxConfig "block_domain_route"
      removeWwpsBoxConfig "block_domain_outbound"
    fi
...
# 读取第三方warp配置
readConfigWarpReg() {
  if [[ ! -f "/etc/wwps/warp/config" ]]; then
    /etc/wwps/warp/warp-reg >/etc/wwps/warp/config
  fi

  secretKeyWarpReg=$(grep <"/etc/wwps/warp/config" private_key | awk '{print $2}')

  addressWarpReg=$(grep <"/etc/wwps/warp/config" v6 | awk '{print $2}')

  publicKeyWarpReg=$(grep <"/etc/wwps/warp/config" public_key | awk '{print $2}')

  reservedWarpReg=$(grep <"/etc/wwps/warp/config" reserved | awk -F "[:]" '{print $2}')

}
# 安装warp-reg工具
installWarpReg() {
  if [[ ! -f "/etc/wwps/warp/warp-reg" ]]; then
    echo
    echoContent yellow "# 注意事项"
    ...
    if [[ ${installWarpRegStatus} == "y" ]]; then
      ...
      curl -sLo /etc/wwps/warp/warp-reg "https://github.com/badafans/warp-reg/releases/download/v1.0/${warpRegCoreCPUVendor}"
      chmod 655 /etc/wwps/warp/warp-reg
...
showWireGuardDomain() {
  ...
  # wwps-box
  if [[ -n ${wwpsBoxConfigPath} ]]; then
    if [[ -f "${wwpsBoxConfigPath}wireguard_endpoints_${type}_route.json" ]]; then
      echoContent yellow "wwps-box"
      jq -r -c '.route.rules[]' "${wwpsBoxConfigPath}wireguard_endpoints_${type}_route.json" | jq -r
    elif [[ ! -f "${wwpsBoxConfigPath}wireguard_endpoints_${type}_route.json" && -f "${wwpsBoxConfigPath}wireguard_endpoints_${type}.json" ]]; then
      echoContent yellow "wwps-box"
      echoContent green " ---> 已设置warp ${type}全局分流"
    else
      echoContent yellow " ---> 未安装warp ${type}分流"
...
addWireGuardRoute() {
  ...
  if [[ -n ${wwpsBoxConfigPath} ]]; then
    ...
    # rule
    addWwpsBoxRouteRule "wireguard_endpoints_${type}" "${domainList}" "wireguard_endpoints_${type}_route"
    # addWwpsBoxOutbound "wireguard_out_${type}" "wireguard_out"
    if [[ -n ${domainList} ]]; then
      addWwpsBoxOutbound "01_direct_outbound"
    fi

    # outbound
    addWwpsBoxWireGuardEndpoints "${type}"
  fi
}
...
unInstallWireGuard() {
  ...
  if [[ ${coreInstallType} == "1" ]]; then
    ...
    if [[ ${type} == "IPv4" ]]; then
      if [[ ! -f "${configPath}wireguard_out_IPv6.json" ]]; then
        rm -rf /etc/wwps/warp/config >/dev/null 2>&1
      fi
    elif [[ ${type} == "IPv6" ]]; then
  ...
  if [[ -n ${wwpsBoxConfigPath} ]]; then
    if [[ ! -f "${wwpsBoxConfigPath}wireguard_endpoints_IPv6_route.json" && ! -f "${wwpsBoxConfigPath}wireguard_endpoints_IPv4_route.json" ]]; then
      rm "${wwpsBoxConfigPath}wireguard_outbound.json" >/dev/null 2>&1
      rm -rf /etc/wwps/warp/config >/dev/null 2>&1
    fi
  fi
}
...
removeWireGuardRoute() {
  ...
  if [[ ${coreInstallType} == "1" ]]; then
    ...
    removeWwpsCoreOutbound "wireguard_out_${type}"
  ...
  # wwps-box
  if [[ -n ${wwpsBoxConfigPath} ]]; then
    removeWwpsBoxRouteRule "wireguard_endpoints_${type}"
  fi
...
warpRoutingReg() {
  ...
  if [[ ${warpStatus} == "1" ]]; then
    ...
    echoContent yellow "# 注意事项"
    echoContent yellow "# 支持wwps-box、wwps-core"
    ...
    if [[ -n ${wwpsBoxConfigPath} ]]; then
      removeWwpsBoxConfig "wireguard_endpoints_${type}_route"

      removeWwpsBoxConfig "wireguard_endpoints_${type}"
      addWwpsBoxOutbound "01_direct_outbound"
    fi
...
socks5InboundRoutingMenu() {
  ...
  1)
    ...
    ;;
  2)
    showWwpsBoxRoutingRules socks5_02_inbound_route
    socks5InboundRoutingMenu
    ;;
  3)
    ...
    ;;
  4)
    if [[ -f "${wwpsBoxConfigPath}20_socks5_inbounds.json" ]]; then
      echoContent yellow "\n ---> 下列内容需要配置到其他机器的出站，请不要进行代理行为\n"
      echoContent green " 端口：$(jq .inbounds[0].listen_port ${wwpsBoxConfigPath}20_socks5_inbounds.json)"
      echoContent green " 用户名称：$(jq -r .inbounds[0].users[0].username ${wwpsBoxConfigPath}20_socks5_inbounds.json)"
      echoContent green " 用户密码：$(jq -r .inbounds[0].users[0].password ${wwpsBoxConfigPath}20_socks5_inbounds.json)"
    else
...
socks5OutboundRoutingMenu() {
  ...
  2)
    ...
    ;;
  3)
    showWwpsBoxRoutingRules socks5_01_outbound_route
...
setSocks5OutboundRoutingAll() {
  ...
  if [[ ${socksOutStatus} == "y" ]]; then
    ...
    if [[ -n ${wwpsBoxConfigPath} ]]; then
      ...
      removeWwpsBoxConfig socks5_01_outbound_route
      removeWwpsBoxConfig 01_direct_outbound
    fi
...
# socks5 分流规则
showWwpsBoxRoutingRules() {
  if [[ -n ${wwpsBoxConfigPath} ]]; then
    if [[ -f "${wwpsBoxConfigPath}$1.json" ]]; then
      jq .route.rules "${wwpsBoxConfigPath}$1.json"
    elif [[ $1 == "socks5_01_outbound_route" && -f "${wwpsBoxConfigPath}socks5_outbound.json" ]]; then
      echoContent yellow "已安装 wwps-box socks5全局出站分流"
      echoContent yellow "\n出站分流配置："
      echoContent skyBlue "$(jq .outbounds[0] ${wwpsBoxConfigPath}socks5_outbound.json)"
    elif [[ $1 == "socks5_02_inbound_route" && -f "${wwpsBoxConfigPath}20_socks5_inbounds.json" ]]; then
      echoContent yellow "已安装 wwps-box socks5全局入站分流"
      echoContent yellow "\n出站分流配置："
      echoContent skyBlue "$(jq .outbounds[0] ${wwpsBoxConfigPath}socks5_outbound.json)"
    fi
  fi
...
removeSocks5Routing() {
  ...
  if [[ ${unInstallSocks5RoutingStatus} == "1" ]]; then
    ...
    if [[ -n ${wwpsBoxConfigPath} ]]; then
      removeWwpsBoxConfig socks5_outbound
      removeWwpsBoxConfig socks5_01_outbound_route
      addWwpsBoxOutbound 01_direct_outbound
    fi
    ...
    removeWwpsBoxConfig 20_socks5_inbounds
    removeWwpsBoxConfig socks5_02_inbound_route
...
setSocks5Inbound() {
  ...
  echoContent skyBlue "\n开始配置Socks5协议入站端口"
  echo
  mapfile -t result < <(initWwpsBoxPort "${wwpsBoxSocks5Port}")
  echoContent green "\n ---> 入站Socks5端口：${result[-1]}"
  ...
  if [[ -z ${socks5RoutingUUID} ]]; then
    if [[ ${coreInstallType} == "1" ]]; then
      socks5RoutingUUID=$(/etc/wwps/wwps-core/wwps-core uuid)
    elif [[ -n ${wwpsBoxConfigPath} ]]; then
      socks5RoutingUUID=$(/etc/wwps/wwps-box/wwps-box generate uuid)
    fi
  fi
  echo
  ...
  cat <<EOF >/etc/wwps/wwps-box/conf/config/20_socks5_inbounds.json
{
    "inbounds":[
        {
          "type": "socks",
...
setSocks5InboundRouting() {
  ...
  if [[ $1 == "addRules" && ! -f "${wwpsBoxConfigPath}socks5_02_inbound_route.json" && ! -f "${configPath}09_routing.json" ]]; then
  ...
  if [[ $1 == "addRules" ]]; then
    socks5InboundRoutingIPs=$(jq .route.rules[0].source_ip_cidr "${wwpsBoxConfigPath}socks5_02_inbound_route.json")
  else
    echoContent red "=============================================================="
  ...
  echoContent yellow "支持wwps-core geosite匹配，支持wwps-box1.8+ rule_set匹配\n"
  ...
  if [[ ${socks5InboundRoutingDomainStatus} == "y" ]]; then
    addWwpsBoxRouteRule "01_direct_outbound" "" "socks5_02_inbound_route"
    local route=
    route=$(jq '.route.rules[0].inbound = ["socks5_inbound"]' "${wwpsBoxConfigPath}socks5_02_inbound_route.json")
    route=$(echo "${route}" | jq ".route.rules[0].source_ip_cidr=${socks5InboundRoutingIPs}")
    echo "${route}" | jq . >"${wwpsBoxConfigPath}socks5_02_inbound_route.json"

    addWwpsBoxOutbound block
    addWwpsBoxOutbound "01_direct_outbound"
  else
    echoContent yellow "录入示例:netflix,openai,google.com\n"
    read -r -p "域名:" socks5InboundRoutingDomain
    if [[ -z ${socks5InboundRoutingDomain} ]]; then
      echoContent red " ---> 域名不可为空"
      exit 0
    fi
    addWwpsBoxRouteRule "01_direct_outbound" "${socks5InboundRoutingDomain}" "socks5_02_inbound_route"
    local route=
    route=$(jq '.route.rules[0].inbound = ["socks5_inbound"]' "${wwpsBoxConfigPath}socks5_02_inbound_route.json")
    route=$(echo "${route}" | jq ".route.rules[0].source_ip_cidr=${socks5InboundRoutingIPs}")
    echo "${route}" | jq . >"${wwpsBoxConfigPath}socks5_02_inbound_route.json"
...
setSocks5Outbound() {
  ...
  echo
  if [[ -n ${wwpsBoxConfigPath} ]]; then
    cat <<EOF >"${wwpsBoxConfigPath}socks5_outbound.json"
{
    "outbounds":[
        {
          "type": "socks",
...
setSocks5OutboundRouting() {
  ...
  if [[ $1 == "addRules" && ! -f "${wwpsBoxConfigPath}socks5_01_outbound_route.json" && ! -f "${configPath}09_routing.json" ]]; then
  ...
  addWwpsBoxRouteRule "socks5_outbound" "${socks5RoutingOutboundDomain}" "socks5_01_outbound_route"
  addWwpsBoxOutbound "01_direct_outbound"
...
setVMessWSRoutingOutbounds() {
  ...
  if [[ -n ${setVMessWSTLSAddress} ]]; then
    removeWwpsCoreOutbound VMess-out
...
reloadCore() {
  ...
  if echo "${currentInstallProtocolType}" | grep -q ",20," || [[ ${coreInstallType} == "2" || -n ${wwpsBoxConfigPath} ]]; then
    handleWwpsBox stop
    handleWwpsBox start
  fi
}
...
sniRouting() {
  ...
  echoContent yellow "# wwps-box不支持规则集，仅支持指定域名。\n"
...
EOF
    ...
    if [[ -n ${wwpsBoxConfigPath} ]]; then
      ...
      addWwpsBoxDNSConfig "${setSNIP}" "${singboxDomainList}" "predefined"
    fi
...
addWwpsBoxDNSConfig() {
  ...
  local rules=
  rules=$(initWwpsBoxRules "${domainList}" "dns")
  # domain精确匹配规则
  local domainRules=
  domainRules=$(echo "${rules}" | jq .domainRules)
  ...
  if [[ -n ${wwpsBoxConfigPath} ]]; then
    if [[ ${actionType} == "predefined" ]]; then
      ...
      cat <<EOF >"${wwpsBoxConfigPath}dns.json"
{
  "dns": {
    "servers": [
        {
...
setUnlockDNS() {
  ...
  if [[ -n ${setDNS} ]]; then
    ...
    if [[ -n ${wwpsBoxConfigPath} ]]; then
      addWwpsBoxOutbound 01_direct_outbound
      addWwpsBoxDNSConfig "${setDNS}" "${domainList}"
    fi
...
EOF
  ...
  if [[ ${coreInstallType} == "2" && -f "${wwpsBoxConfigPath}dns.json" ]]; then
    cat <<EOF >${wwpsBoxConfigPath}dns.json
{
    "dns": {
        "servers":[
...
customWwpsBoxInstall() {
  ...
  if [[ ${selectCustomInstallType//,/} =~ ^[0-9]+$ ]]; then
    ...
    installWwpsBox 4
    installWwpsBoxService 5
    initWwpsBoxConfig custom 6
    cleanUp xrayDel
    installCronTLS 7
    handleWwpsBox stop
    handleWwpsBox start
...
# 一键无域名wwps-core Reality
installWwpsCoreReality() {
  ...
  # 安装Xray
  installWwpsCore 2 false
  installWwpsCoreService 3
  initWwpsCoreConfig custom 4
  cleanUp wwpsBoxDel
...
# 一键无域名wwps-box Reality
installWwpsBoxReality() {
...
customWwpsCoreInstall() {
  ...
  if [[ ${selectCustomInstallType//,/} =~ ^[0-7]+$ ]]; then
    ...
    # 安装Xray
    installWwpsCore 7 false
    installWwpsCoreService 8
    initWwpsCoreConfig custom 9
    cleanUp wwpsBoxDel
    if [[ ${selectCustomInstallType} != ",7," ]]; then
      installCronTLS 10
    fi
...
# 选择核心安装wwps-box、wwps-core
selectCoreInstall() {
  echoContent skyBlue "\n功能 1/${totalProgress} : 选择核心安装"
  echoContent red "\n=============================================================="
  echoContent yellow "1.wwps-core"
  echoContent yellow "2.wwps-box"
  echoContent red "=============================================================="
...
xrayCoreInstall() {
  ...
  # 安装Xray
  installWwpsCore 6 false
  installWwpsCoreService 7
  initWwpsCoreConfig all 8
  cleanUp wwpsBoxDel
  installCronTLS 9
  if [[ -n ${btDomain} ]]; then
...
wwpsBoxInstall() {
  ...
  installWwpsBox 5
  installWwpsBoxService 6
  initWwpsBoxConfig all 7
...
cronFunction() {
  if [[ ${cronName} == "RenewTLS" ]]; then
    ...
    updateGeoSite >>/etc/wwps/crontab_updateGeoSite.log
    echoContent green " ---> geo更新日期:$(date "+%F %H:%M:%S")" >>/etc/wwps/crontab_updateGeoSite.log
    exit 0
  fi
}
# 账号管理
...
installSubscribe() {
  ...
  if [[ -z ${subscribePort} ]]; then
    ...
    mapfile -t result < <(initWwpsBoxPort "${subscribePort}")
    echo
    ...
    if [[ ${httpSubscribeStatus} == "true" ]]; then
      ...
      nginxSubscribeSSL="ssl_certificate /etc/wwps/tls/${subscribeServerName}.crt;ssl_certificate_key /etc/wwps/tls/${subscribeServerName}.key;"
    fi
...
server {
    ...
    client_max_body_size 100m;
    root ${nginxStaticPath};
    location ~ ^/s/(clashMeta|default|clashMetaProfiles|wwps-box|wwps-box_profiles)/(.*) {
        default_type 'text/plain; charset=utf-8';
        alias /etc/wwps/subscribe/\$1/\$2;
    }
    location / {
    }
}
...
addSubscribeMenu() {
  ...
  if [[ ${addSubscribeStatus} == "1" ]]; then
    ...
    if [[ ! -f "/etc/wwps/subscribe_remote/remoteSubscribeUrl" ]]; then
      echoContent green " ---> 未安装其他订阅"
      exit 0
    fi
    grep -v '^$' "/etc/wwps/subscribe_remote/remoteSubscribeUrl" | awk '{print NR""":"$0}'
    ...
    sed -i "$((delSubscribeIndex))d" "/etc/wwps/subscribe_remote/remoteSubscribeUrl" >/dev/null 2>&1
...
addOtherSubscribe() {
  ...
  echoContent skyBlue "录入示例：www.google.com:443:vps1\n"
  ...
  if [[ -z ${remoteSubscribeUrl} ]]; then
    ...
    if [[ -f "/etc/wwps/subscribe_remote/remoteSubscribeUrl" ]] && grep -q "${remoteSubscribeUrl}" /etc/wwps/subscribe_remote/remoteSubscribeUrl; then
    ...
    echo "${remoteSubscribeUrl}" >>/etc/wwps/subscribe_remote/remoteSubscribeUrl
    subscribe
  fi
}
# clashMeta配置文件
clashMetaConfig() {
  local url=$1
  local id=$2
  cat <<EOF >"/etc/wwps/subscribe/clashMetaProfiles/${id}"
log-level: debug
mode: rule
ipv6: true
mixed-port: 7890
...
subscribe() {
  ...
  if [[ ${coreInstallType} == "1" || ${coreInstallType} == "2" ]]; then
    ...
    if [[ -f "/etc/wwps/subscribe_local/subscribeSalt" && -n $(cat "/etc/wwps/subscribe_local/subscribeSalt") ]]; then
      if [[ -z ${renewSalt} ]]; then
        read -r -p "读取到上次安装设置的Salt，是否使用上次生成的Salt ？[y/n]:" historySaltStatus
        if [[ ${historySaltStatus} == "y" ]]; then
          subscribeSalt=$(cat /etc/wwps/subscribe_local/subscribeSalt)
        else
    ...
    echo "${subscribeSalt}" >/etc/wwps/subscribe_local/subscribeSalt

    rm -rf /etc/wwps/subscribe/default/*
    rm -rf /etc/wwps/subscribe/clashMeta/*
    rm -rf /etc/wwps/subscribe_local/default/*
    rm -rf /etc/wwps/subscribe_local/clashMeta/*
    rm -rf /etc/wwps/subscribe_local/wwps-box/*
    showAccounts >/dev/null
    if [[ -n $(ls /etc/wwps/subscribe_local/default/) ]]; then
      if [[ -f "/etc/wwps/subscribe_remote/remoteSubscribeUrl" && -n $(cat "/etc/wwps/subscribe_remote/remoteSubscribeUrl") ]]; then
        if [[ -z ${renewSalt} ]]; then
      ...
      find /etc/wwps/subscribe_local/default/* | while read -r email; do
        email=$(echo "${email}" | awk -F "[d][e][f][a][u][l][t][/]" '{print $2}')

        local emailMd5=
        emailMd5=$(echo -n "${email}${subscribeSalt}"$'\n' | md5sum | awk '{print $1}')

        cat "/etc/wwps/subscribe_local/default/${email}" >>"/etc/wwps/subscribe/default/${emailMd5}"
        if [[ ${updateOtherSubscribeStatus} == "y" ]]; then
          updateRemoteSubscribe "${emailMd5}" "${email}"
        fi
        local base64Result
        base64Result=$(base64 -w 0 "/etc/wwps/subscribe/default/${emailMd5}")
        echo "${base64Result}" >"/etc/wwps/subscribe/default/${emailMd5}"
        echoContent yellow "--------------------------------------------------------------"
        ...
        if [[ -z ${showStatus} ]]; then
          ...
          # clashMeta
          if [[ -f "/etc/wwps/subscribe_local/clashMeta/${email}" ]]; then

            cat "/etc/wwps/subscribe_local/clashMeta/${email}" >>"/etc/wwps/subscribe/clashMeta/${emailMd5}"

            sed -i '1i\proxies:' "/etc/wwps/subscribe/clashMeta/${emailMd5}"
          ...
          # wwps-box
          if [[ -f "/etc/wwps/subscribe_local/wwps-box/${email}" ]]; then
            cp "/etc/wwps/subscribe_local/wwps-box/${email}" "/etc/wwps/subscribe/wwps-box_profiles/${emailMd5}"

            echoContent skyBlue " ---> 下载 wwps-box 通用配置文件"
            if [[ ${release} == "alpine" ]]; then
              wget -O "/etc/wwps/subscribe/wwps-box/${emailMd5}" -q "https://raw.githubusercontent.com/youugiuhiuh/Wuthering_Waves_Private_Server/master/fodder/wwps-box.json"
            else
              wget -O "/etc/wwps/subscribe/wwps-box/${emailMd5}" -q "${wgetShowProgressStatus}" "https://raw.githubusercontent.com/youugiuhiuh/Wuthering_Waves_Private_Server/master/fodder/wwps-box.json"
            fi

            jq ".outbounds=$(jq ".outbounds|map(if has(\"outbounds\") then .outbounds += $(jq ".|map(.tag)" "/etc/wwps/subscribe_local/wwps-box/${email}") else . end)" "/etc/wwps/subscribe/wwps-box/${emailMd5}")" "/etc/wwps/subscribe/wwps-box/${emailMd5}" >"/etc/wwps/subscribe/wwps-box/${emailMd5}_tmp" && mv "/etc/wwps/subscribe/wwps-box/${emailMd5}_tmp" "/etc/wwps/subscribe/wwps-box/${emailMd5}"
            jq ".outbounds += $(jq '.' "/etc/wwps/subscribe_local/wwps-box/${email}")" "/etc/wwps/subscribe/wwps-box/${emailMd5}" >"/etc/wwps/subscribe/wwps-box/${emailMd5}_tmp" && mv "/etc/wwps/subscribe/wwps-box/${emailMd5}_tmp" "/etc/wwps/subscribe/wwps-box/${emailMd5}"

            echoContent skyBlue "\n----------wwps-box订阅----------\n"
            echoContent yellow "url:${subscribeType}://${currentDomain}/s/wwps-box/${emailMd5}\n"
            echoContent yellow "在线二维码:https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=${subscribeType}://${currentDomain}/s/wwps-box/${emailMd5}\n"
            if [[ ${release} != "alpine" ]]; then
              echo "${subscribeType}://${currentDomain}/s/wwps-box/${emailMd5}" | qrencode -s 10 -m 1 -t UTF8
            fi
...
updateRemoteSubscribe() {
  ...
  while read -r line; do
    ...
    if ! echo "${clashMetaProxies}" | grep -q "nginx" && [[ -n ${clashMetaProxies} ]]; then
      echo "${clashMetaProxies}" >>"/etc/wwps/subscribe/clashMeta/${emailMD5}"
    ...
    if ! echo "${default}" | grep -q "nginx" && [[ -n ${default} ]]; then
      ...
      echo "${default}" >>"/etc/wwps/subscribe/default/${emailMD5}"
    ...
    local wwpsBoxSubscribe=
    wwpsBoxSubscribe=$(curl -s "${subscribeType}://${remoteUrl}/s/wwps-box_profiles/${emailMD5}")

    if ! echo "${wwpsBoxSubscribe}" | grep -q "nginx" && [[ -n ${wwpsBoxSubscribe} ]]; then
      wwpsBoxSubscribe=${wwpsBoxSubscribe//tag\": \"${email}/tag\": \"${email}_${serverAlias}}
      wwpsBoxSubscribe=$(jq ". +=${wwpsBoxSubscribe}" "/etc/wwps/subscribe_local/wwps-box/${email}")
      echo "${wwpsBoxSubscribe}" | jq . >"/etc/wwps/subscribe_local/wwps-box/${email}"
  ...
  done < <(grep -v '^$' <"/etc/wwps/subscribe_remote/remoteSubscribeUrl")
}
...
initRealityKey() {
  ...
  if [[ -z ${realityPrivateKey} ]]; then
    if [[ ${selectCoreType} == "2" || ${coreInstallType} == "2" ]]; then
      realityX25519Key=$(/etc/wwps/wwps-box/wwps-box generate reality-keypair)
      realityPrivateKey=$(echo "${realityX25519Key}" | head -1 | awk '{print $2}')
      realityPublicKey=$(echo "${realityX25519Key}" | tail -n 1 | awk '{print $2}')
      echo "publicKey:${realityPublicKey}" >/etc/wwps/wwps-box/conf/config/reality_key
    else
      read -r -p "请输入Private Key[回车自动生成]:" historyPrivateKey
      if [[ -n ${historyPrivateKey} ]]; then
        realityX25519Key=$(/etc/wwps/wwps-core/wwps-core x25519 -i "${historyPrivateKey}")
      else
        realityX25519Key=$(/etc/wwps/wwps-core/wwps-core x25519)
      fi
...
initRealityMldsa65() {
  echoContent skyBlue "\n生成Reality mldsa65\n"
  if /etc/wwps/wwps-core/wwps-core tls ping "${realityServerName}:${realityDomainPort}" 2>/dev/null | grep -q "X25519MLKEM768"; then
    length=$(/etc/wwps/wwps-core/wwps-core tls ping "${realityServerName}:${realityDomainPort}" | grep "Certificate chain's total length:" | awk '{print $5}' | head -1)
    ...
    if [ "$length" -gt 3500 ]; then
      ...
      if [[ -z ${realityMldsa65Seed} ]]; then
        ...
        #            echo "publicKey:${realityPublicKey}" >/etc/wwps/wwps-box/conf/config/reality_key
        #        else
        realityMldsa65=$(/etc/wwps/wwps-core/wwps-core mldsa65)
...
# 初始化reality端口
initWwpsCoreRealityPort() {
  if [[ -n ${wwpsCoreVLESSRealityPort} && -z ${lastInstallationConfig} ]]; then
    read -r -p "读取到上次安装记录，是否使用上次安装时的端口 ？[y/n]:" historyRealityPortStatus
    if [[ ${historyRealityPortStatus} == "y" ]]; then
      realityPort=${wwpsCoreVLESSRealityPort}
    fi
  elif [[ -n ${wwpsCoreVLESSRealityPort} && -n ${lastInstallationConfig} ]]; then
    realityPort=${wwpsCoreVLESSRealityPort}
  fi
  ...
  if [[ -z ${realityPort} ]]; then
    ...
    #        fi
    if [[ -n ${realityPort} && ${wwpsCoreVLESSRealityPort} == "${realityPort}" ]]; then
      handleWwpsCore stop
    else
...
# 初始化XHTTP端口
initWwpsCoreXHTTPort() {
  if [[ -n ${wwpsCoreVLESSRealityXHTTPort} && -z ${lastInstallationConfig} ]]; then
    read -r -p "读取到上次安装记录，是否使用上次安装时的端口 ？[y/n]:" historyXHTTPortStatus
    if [[ ${historyXHTTPortStatus} == "y" ]]; then
      xHTTPort=${wwpsCoreVLESSRealityXHTTPort}
    fi
  elif [[ -n ${wwpsCoreVLESSRealityXHTTPort} && -n ${lastInstallationConfig} ]]; then
    xHTTPort=${wwpsCoreVLESSRealityXHTTPort}
  fi
  ...
  if [[ -z ${xHTTPort} ]]; then
    ...
    if [[ -n ${xHTTPort} && ${wwpsCoreVLESSRealityXHTTPort} == "${xHTTPort}" ]]; then
      handleWwpsCore stop
    else
      checkPort "${xHTTPort}"
    fi
...
manageReality() {
  ...
  if [[ ${coreInstallType} == "1" ]]; then
    selectCustomInstallType=",7,"
    initWwpsCoreConfig custom 1 true
  elif [[ ${coreInstallType} == "2" ]]; then
    if echo "${currentInstallProtocolType}" | grep -q ",7,"; then
      selectCustomInstallType=",7,"
    fi
    ...
    initWwpsBoxConfig custom 1 true
  fi
...
installRealityScanner() {
  if [[ ! -f "/etc/wwps/wwps-core/reality_scan/RealiTLScanner-linux-64" ]]; then
    version=$(curl -s https://api.github.com/repos/XTLS/RealiTLScanner/releases?per_page=1 | jq -r '.[]|.tag_name')
    wget -c -q -P /etc/wwps/wwps-core/reality_scan/ "https://github.com/XTLS/RealiTLScanner/releases/download/${version}/RealiTLScanner-linux-64"
    chmod 655 /etc/wwps/wwps-core/reality_scan/RealiTLScanner-linux-64
  fi
}
# reality scanner
realityScanner() {
  ...
  if [[ ${ipStatus} == "y" ]]; then
    echoContent yellow "结果存储在 /etc/wwps/wwps-core/reality_scan/result.log 文件中\n"
    /etc/wwps/wwps-core/reality_scan/RealiTLScanner-linux-64 -addr "${publicIP}" | tee /etc/wwps/wwps-core/reality_scan/result.log
  else
...
manageHysteria() {
  ...
  if [[ -n ${wwpsBoxConfigPath} ]] && [[ -f "/etc/wwps/wwps-box/conf/config/06_hysteria2_inbounds.json" ]]; then
    echoContent yellow "依赖第三方wwps-box\n"
    echoContent yellow "1.重新安装"
    echoContent yellow "2.卸载"
    echoContent yellow "3.端口跳跃管理"
    hysteria2Status=true
  else
    echoContent yellow "依赖wwps-box内核\n"
...
manageTuic() {
  ...
  echoContent red "\n=============================================================="
  local tuicStatus=
  if [[ -n ${wwpsBoxConfigPath} ]] && [[ -f "/etc/wwps/wwps-box/conf/config/09_tuic_inbounds.json" ]]; then
    echoContent yellow "依赖wwps-box内核\n"
    echoContent yellow "1.重新安装"
    echoContent yellow "2.卸载"
    echoContent yellow "3.端口跳跃管理"
...
# wwps-box log日志
wwpsBoxLog() {
  cat <<EOF >/etc/wwps/wwps-box/conf/config/log.json
{
  "log": {
    "disabled": $1,
    "level": "debug",
    "output": "/etc/wwps/wwps-box/conf/box.log",
    "timestamp": true
  }
}
EOF
...
# wwps-box 版本管理
wwpsBoxVersionManageMenu() {
  echoContent skyBlue "\n进度  $1/${totalProgress} : wwps-box 版本管理"
  if [[ -z ${wwpsBoxConfigPath} ]]; then
    echoContent red " ---> 没有检测到安装程序，请执行脚本安装内容"
    menu
    exit 0
  fi
  echoContent red "\n=============================================================="
  echoContent yellow "1.升级 wwps-box"
  echoContent yellow "2.关闭 wwps-box"
  echoContent yellow "3.打开 wwps-box"
  echoContent yellow "4.重启 wwps-box"
  echoContent yellow "=============================================================="
  local logStatus=
  if [[ -n ${wwpsBoxConfigPath} && -f "${wwpsBoxConfigPath}log.json" && "$(jq -r .log.disabled "${wwpsBoxConfigPath}log.json")" == "false" ]]; then
  ...
  read -r -p "请选择:" selectWwpsBoxType
  if [[ ! -f "${wwpsBoxConfigPath}../box.log" ]]; then
    touch "${wwpsBoxConfigPath}../box.log" >/dev/null 2>&1
  fi
  if [[ ${selectWwpsBoxType} == "1" ]]; then
    installWwpsBox 1
    handleWwpsBox stop
    handleWwpsBox start
  elif [[ ${selectWwpsBoxType} == "2" ]]; then
    handleWwpsBox stop
  elif [[ ${selectWwpsBoxType} == "3" ]]; then
    handleWwpsBox start
  elif [[ ${selectWwpsBoxType} == "4" ]]; then
    handleWwpsBox stop
    handleWwpsBox start
  elif [[ ${selectWwpsBoxType} == "5" ]]; then
    wwpsBoxLog ${logStatus}
    if [[ ${logStatus} == "false" ]]; then
      tail -f "${wwpsBoxConfigPath}../box.log"
    fi
  elif [[ ${selectWwpsBoxType} == "6" ]]; then
    tail -f "${wwpsBoxConfigPath}../box.log"
  fi
}
# TG Bot管理
manageTGBot() {
  echoContent skyBlue "\n进度 [1/1] -> TG Bot 管理"
  echoContent yellow "1. 安装/更新 TG Bot"
  echoContent yellow "2. 卸载 TG Bot"
  echoContent yellow "0. 返回主菜单"
  read -r -p "请选择管理操作:" tgbot_op
  case ${tgbot_op} in
  1)
    echoContent skyBlue "\n开始下载 TG Bot..."

    if [[ ${release} != "centos" ]]; then
      echoContent skyBlue "更新软件索引..."
      ${upgrade} >/dev/null 2>&1
    fi

    # 获取最新版本号 (优先从 GitHub 获取最新 Release)
    local github_version
    github_version=$(curl -s "https://api.github.com/repos/youugiuhiuh/Wuthering_Waves_Private_Server/releases/latest" | grep -oP '"tag_name": "\K[^"]+')

    if [[ -z ${github_version} ]]; then
      # 如果 API 失败，尝试从当前运行的脚本解析
      github_version=$(grep -oP '当前版本：\Kv[0-9]+\.[0-9]+\.[0-9]+' "$0" 2>/dev/null || echo "v0.1.8")
    fi
    ...
    # 下载预编译的 tgbot 二进制文件
    echoContent yellow "正在从 GitHub Releases 下载 TG Bot ${github_version}..."
    if ! wget -q -P "${temp_dir}" "https://github.com/youugiuhiuh/Wuthering_Waves_Private_Server/releases/download/${github_version}/tgbot"; then
      echoContent red "下载失败，请检查网络连接或 GitHub 访问权限"
      rm -rf "${temp_dir}"
      return
    fi

    # 验证下载的文件
    if [[ ! -f "${temp_dir}/tgbot" ]]; then
      echoContent red "下载的文件不存在"
      rm -rf "${temp_dir}"
      return
    fi

    # 获取 GitHub Release 的 SHA256 值进行验证
    echoContent yellow "验证文件完整性..."
    local expected_sha256
    expected_sha256=$(curl -s "https://api.github.com/repos/youugiuhiuh/Wuthering_Waves_Private_Server/releases/tags/${github_version}" | grep -oP '"digest": "\K[^"]+' | grep -oP 'sha256:\K.*' || echo "")

    if [[ -n ${expected_sha256} ]]; then
      local actual_sha256
      actual_sha256=$(sha256sum "${temp_dir}/tgbot" | awk '{print $1}')

      if [[ ${actual_sha256} != "${expected_sha256}" ]]; then
        echoContent red "文件校验失败！"
        echoContent yellow "期望 SHA256: ${expected_sha256}"
        echoContent yellow "实际 SHA256: ${actual_sha256}"
        echoContent red "文件可能已损坏或被篡改，下载失败"
        rm -rf "${temp_dir}"
        return
      fi
      echoContent green "✓ 文件完整性验证通过"
    else
      echoContent yellow "警告：无法获取 SHA256 校验值，跳过完整性验证"
    fi

    # 安装二进制文件
    mkdir -p /etc/wwps/tgbot
    systemctl stop wwps-tgbot >/dev/null 2>&1
    sleep 1
    cp -f "${temp_dir}/tgbot" /etc/wwps/tgbot/tgbot
    chmod +x /etc/wwps/tgbot/tgbot

    # 清理临时目录
    rm -rf "${temp_dir}"

    echoContent green "TG Bot 下载完成！"

    if [[ -f "/etc/wwps/tgbot/config.enc" ]]; then
      echoContent green "\n检测到已存在配置文件，跳过初始化设置。"
    else
      echoContent skyBlue "\n配置 TG Bot..."
      read -r -p "请输入 TG Bot Token:" bot_token
      read -r -p "请输入管理员 ID (TG User ID):" admin_id

      local totp_secret
      totp_secret=$(/etc/wwps/tgbot/tgbot --generate-totp-secret)
      echoContent yellow "\n--- 重要: TOTP 绑定 ---"
      echoContent yellow "您的 TOTP 密钥为: ${totp_secret}"
      # 构造 otpauth URL，算法与应用保持一致 (SHA512, 6 位, 30 秒)
      local totp_label="wwps:admin"
      local totp_issuer="wwps"
      local otpauth_url="otpauth://totp/${totp_label}?secret=${totp_secret}&issuer=${totp_issuer}&algorithm=SHA512&digits=6&period=30"

      if ! command -v qrencode >/dev/null 2>&1; then
        echoContent yellow "未检测到 qrencode，尝试自动安装..."
        if ${installType} qrencode >/dev/null 2>&1; then
          echoContent green "qrencode 安装成功。"
        else
          echoContent red "qrencode 自动安装失败，无法输出二维码。请手动安装后运行: qrencode -t ANSIUTF8 '${otpauth_url}'"
        fi
      fi

      if command -v qrencode >/dev/null 2>&1; then
        echoContent yellow "二维码 (请使用支持 SHA512 的 TOTP 客户端扫码)："
        qrencode -t ANSIUTF8 "${otpauth_url}"
      else
        echoContent yellow "建议使用 Google Authenticator / Microsoft Authenticator 扫码或手动添加。"
        echoContent yellow "如需二维码，可先安装 qrencode 再运行: qrencode -t ANSIUTF8 '${otpauth_url}'"
      fi

      echoContent yellow "如无法扫码，可手动添加上方密钥，或使用链接: ${otpauth_url}"
      echoContent yellow "绑定完成后请尽快清屏/关闭终端，避免密钥与 otpauth URL 泄露。"
      echoContent yellow "----------------------\n"

      /etc/wwps/tgbot/tgbot --setup "${bot_token}" "${admin_id}" "${totp_secret}"
    fi

    # 安装 systemd 服务
    cat >/etc/systemd/system/wwps-tgbot.service <<EOF
[Unit]
...
Type=simple
User=root
WorkingDirectory=/etc/wwps/tgbot
ExecStart=/etc/wwps/tgbot/tgbot
Restart=always
RestartSec=5
...
EOF
    systemctl daemon-reload
    systemctl enable wwps-tgbot
    systemctl restart wwps-tgbot
    echoContent green "\nTG Bot 已成功安装并启动！"
    ;;
  2)
    systemctl stop wwps-tgbot
    systemctl disable wwps-tgbot
    rm -f /etc/systemd/system/wwps-tgbot.service
    rm -rf /etc/wwps/tgbot
    echoContent green "\nTG Bot 卸载成功。"
    ;;
  *)
    menu
...
menu() {
  ...
  echoContent yellow "19.TG Bot管理"
