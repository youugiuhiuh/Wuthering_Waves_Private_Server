...
mod tests {
    ...
    #[test]
    fn test_from_code_aes_password_generated() {
        let mask = KcpMask::from_code("ma").unwrap();
        assert!(matches!(mask, KcpMask::MkcpAes128Gcm { .. }));
    }

    #[test]
    fn test_parse_codes_valid() {
        let masks = KcpMask::parse_codes(&["mo", "hw"]).unwrap();
        assert_eq!(masks.len(), 2);
    }

    #[test]
    fn test_parse_codes_invalid() {
        assert!(KcpMask::parse_codes(&["mo", "invalid"]).is_err());
    }
    ...
    fn test_variants_by_category() {
        ...
        let dis = KcpMask::variants_by_category("dis");
    ...
    fn test_validate_stack_valid() {
        assert!(KcpMask::validate_stack(&[KcpMask::MkcpOriginal, KcpMask::HeaderWechat]).is_ok());
    }
    ...
    fn test_canonical_order() {
        ...
        assert!(matches!(&ordered[0], KcpMask::MkcpOriginal));
        assert!(matches!(&ordered[1], KcpMask::Xdns { .. }));
    }

    #[test]
    fn test_get_stack_warnings_alone() {
        let warnings = KcpMask::get_stack_warnings(&[KcpMask::MkcpOriginal]);
        assert!(!warnings.is_empty());
    }

    #[test]
    fn test_get_stack_warnings_with_disguise() {
        let warnings = KcpMask::get_stack_warnings(&[KcpMask::MkcpOriginal, KcpMask::HeaderWechat]);
        assert!(warnings.is_empty());
    }
