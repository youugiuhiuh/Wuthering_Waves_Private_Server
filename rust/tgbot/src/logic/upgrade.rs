...
struct ReleaseAsset {
    ...
    /// Gitea/Codeberg 等可能使用 url 而非 browser_download_url
    #[serde(default)]
    url: String,
    size: Option<u64>,
    #[serde(default)]
...
impl UpgradeManager {
    ...
    pub async fn write_upgrade_flag(&self, version: &str) -> Result<()> {
        let flag_path = format!("{}", obfstr!("/etc/wwps/tgbot/upgrade.flag"));
        if let Some(parent) = Path::new(&flag_path).parent() {
            fs::create_dir_all(parent)
                .await
                .context(format!("{}", obfstr!("创建升级标记目录失败")))?;
        }
        fs::write(&flag_path, version)
            .await
            .context(format!("{}", obfstr!("写入升级标记文件失败")))
    }
