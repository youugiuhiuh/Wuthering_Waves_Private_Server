...
use crate::logic;
use crate::logic::scheduler::{self, TaskType};
use crate::utils;
...
pub async fn handle(ctx: &CallbackContext) -> HandlerResult {
    ...
    match data {
        "m_sched" => {
            ...
            ctx.bot.edit_message_text(ctx.chat_id, ctx.msg_id, summary)
        ...
        d if d.starts_with("s_custom:") => {
            ...
            ctx.bot.edit_message_text(ctx.chat_id, ctx.msg_id, text)
                .parse_mode(ParseMode::Html)
                .reply_markup(build_custom_schedule_keyboard(&ret))
                .await?;
        }
        "s_custom_ui:main" => {
            if let Some((text, ret)) = ctx.state
        ...
        "s_custom_ui:day" => {
            if let Some(is_daily) = ctx.state
                .with_schedule_input(ctx.chat_id, |input| {
                    input.updated_at = Instant::now();
        ...
        "s_custom_ui:hour" => {
            if ctx.state
                ...
                .is_some()
            {
                ctx.bot.edit_message_text(ctx.chat_id, ctx.msg_id, "🕐 <b>选择执行小时</b>")
                    .parse_mode(ParseMode::Html)
        ...
        "s_custom_ui:minute" => {
            if ctx.state
                ...
                .is_some()
            {
                ctx.bot.edit_message_text(ctx.chat_id, ctx.msg_id, "🕑 <b>选择执行分钟</b>")
                    .parse_mode(ParseMode::Html)
        ...
        "s_custom_ui:tz" => {
            if ctx.state
                ...
                .is_some()
            {
                ctx.bot.edit_message_text(ctx.chat_id, ctx.msg_id, "🌍 <b>选择任务时区</b>")
                    .parse_mode(ParseMode::Html)
        ...
        "s_custom_confirm" => {
            let Some((cron, task_type, timezone, return_to)) = ctx.state
                .with_schedule_input(ctx.chat_id, |input| {
                    input.updated_at = Instant::now();
                    (
        ...
        "s_del_menu" => {
            if let Some(manager) = logic::scheduler::get_manager().await {
                ...
                ctx.bot.edit_message_text(ctx.chat_id, ctx.msg_id, "➖ <b>删除任务</b>\n点击移除:")
                    .parse_mode(ParseMode::Html)
        ...
        "geo_sched_off" => {
            if let Some(manager) = logic::scheduler::get_manager().await {
                ...
                drop(state_lock);
                let _ = manager.start_all_tasks(ctx.bot.clone(), ctx.state.admin_id()).await;
