...
use tgbot::core::paths::{xray, singbox};
    ...
    match res {
        ...
        Err(e) => {
            bot.send_message(chat_id, format!("❌ 生成失败: {}", e))
                ...
                "a_fw" => {
                    crate::handlers::security::handle_firewall_harden(bot.clone(), q, chat_id, msg_id).await?;
                }
                "a_upgrade" => {
                    bot.answer_callback_query(q.id.clone())
                        .text("⚙️ 正在启动 Bot 自更新...")
                ...
                "a_geo_sched_menu" => {
                    let geo_info = if let Some(manager) = crate::logic::scheduler::get_manager().await {
                ...
                "a_sys_maint" => {
                    if crate::logic::operations::MAINTENANCE_FLAG.load(std::sync::atomic::Ordering::SeqCst)
                    {
                        bot.answer_callback_query(q.id.clone())
                ...
                "a_sys_reboot" => {
                    if crate::logic::operations::REBOOT_FLAG.load(std::sync::atomic::Ordering::SeqCst) {
                        bot.answer_callback_query(q.id.clone())
                ...
                "s_custom_confirm" => {
                    ...
                    state.remove_schedule_input(chat_id).await;
                    if let Some(manager) = crate::logic::scheduler::get_manager().await {
                        let task = crate::logic::scheduler::ScheduledTask::new_with_timezone(
                            task_type,
                            &cron_expression,
                            &timezone,
                        );
                ...
                d if d.starts_with("s_add:") => {
                    ...
                    if let Some(manager) = crate::logic::scheduler::get_manager().await {
                        let task = crate::logic::scheduler::ScheduledTask::new(task_type, cron);
                        let _ = manager
                            .add_new_task(bot.clone(), state.admin_id(), task)
                            .await;
                        bot.answer_callback_query(q.id.clone())
