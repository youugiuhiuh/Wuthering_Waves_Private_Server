         ...
         ⚠️ 至少选择1层，建议加密层+伪装层组合",
             ...
             ⬇️ <b>请选择生成数量:</b>",
                ...
                d if d.starts_with("s_custom:") => {
                    ...
                    let text = crate::build_custom_schedule_text(&input_state);
                    let ret = input_state.return_to.clone();

                    bot.edit_message_text(chat_id, msg_id, text)
                        .parse_mode(ParseMode::Html)
                        .reply_markup(crate::build_custom_schedule_keyboard(&ret))
                        .await?;
                }
                "s_custom_ui:main" => {
                    if let Some((text, ret)) = state
                        .with_schedule_input(chat_id, |input| {
                            ...
                            (crate::build_custom_schedule_text(input), input.return_to.clone())
                        })
                        .await
                    {
                        bot.edit_message_text(chat_id, msg_id, text)
                ...
                "s_custom_ui:day" => {
                        ...
                        .await
                    {
                        if is_daily {
                            ...
                            bot.edit_message_text(chat_id, msg_id, text)
                                .parse_mode(ParseMode::Html)
                                .reply_markup(crate::build_custom_day_keyboard())
                                .await?;
                        }
                    } else {
                        bot.answer_callback_query(q.id)
                ...
                "s_custom_ui:hour" => {
                        ...
                        .is_some()
                    {
                        bot.edit_message_text(chat_id, msg_id, "🕐 <b>选择执行小时</b>")
                            .parse_mode(ParseMode::Html)
                            .reply_markup(crate::build_custom_hour_keyboard())
                            .await?;
                    } else {
                        bot.answer_callback_query(q.id)
                            .text("⚠️ 自定义定时会话不存在，请重新进入。")
                ...
                "s_custom_ui:minute" => {
                        ...
                        .is_some()
                    {
                        bot.edit_message_text(chat_id, msg_id, "🕑 <b>选择执行分钟</b>")
                            .parse_mode(ParseMode::Html)
                            .reply_markup(crate::build_custom_minute_keyboard())
                            .await?;
                    } else {
                        bot.answer_callback_query(q.id)
                            .text("⚠️ 自定义定时会话不存在，请重新进入。")
                ...
                "s_custom_ui:tz" => {
                        ...
                        .is_some()
                    {
                        bot.edit_message_text(chat_id, msg_id, "🌍 <b>选择任务时区</b>")
                            .parse_mode(ParseMode::Html)
                            .reply_markup(crate::build_custom_timezone_keyboard())
                            .await?;
                    } else {
                        bot.answer_callback_query(q.id)
                            .text("⚠️ 自定义定时会话不存在，请重新进入。")
                ...
                "s_custom_confirm" => {
                    let Some((cron, task_type, timezone, return_to)) = state
                        .with_schedule_input(chat_id, |input| {
                            input.updated_at = Instant::now();
                            (
                                crate::build_cron_from_custom_state(input),
