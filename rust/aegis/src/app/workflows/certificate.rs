...
pub struct CertificateWorkflow {
    pending: Mutex<HashMap<(Platform, ConversationId), DomainInputState>>,
}
...
impl CertificateWorkflow {
    ...
    pub fn transition(
        ...
        match pending.get_mut(&(platform, conversation.clone())) {
...
mod tests {
    ...
    fn start_initializes_await_domain() {
        ...
        let state = wf
            .snapshot(platform, &conversation(1))
    ...
    fn unknown_conversation_has_no_snapshot() {
        let wf = CertificateWorkflow::new();
        let platform = Platform::Telegram;
        assert!(wf.snapshot(platform, &conversation(9)).is_none());
        assert_eq!(
            wf.timeout_status(platform, &conversation(9), Duration::from_secs(120)),
    ...
    fn timeout_status_expires_stale_flow() {
        ...
        assert_eq!(
            wf.timeout_status(platform, &conversation(1), Duration::from_secs(120)),
    ...
    fn take_removes_flow() {
        ...
        assert!(wf.take(platform, &conversation(1)).is_some());
        assert!(wf.snapshot(platform, &conversation(1)).is_none());
    }
    ...
    fn different_platforms_are_separate_flows() {
        ...
        let tg_state = wf
            .snapshot(Platform::Telegram, &conversation(1))
            .expect("state present");
        let dc_state = wf
            .snapshot(Platform::Discord, &conversation(1))
            .expect("state present");
        assert_eq!(tg_state.source, DomainFlowSource::OneClick);
        assert_eq!(dc_state.source, DomainFlowSource::Standalone);
    }
}
