...
checkCentosSELinux() {
  if [[ -f "/etc/selinux/config" ]] && ! grep -q "SELINUX=disabled" <"/etc/selinux/config"; then
    ...
    echoContent yellow "It is detected that SELinux is turned on. Please turn it off manually."
    exit 0
  fi
}
checkSystem() {
...
initVar() {
  ...
  # Path to v2ray-core, wwps-core configuration files
  configPath=

  # wwps-core reality state
  realityStatus=
...
readInstallType() {
  ...
  if [[ -d "/etc/wwps" ]]; then
    ...
    if [[ -d "/etc/wwps/v2ray" && -f "/etc/wwps/v2ray/v2ray" && -f "/etc/wwps/v2ray/v2ctl" ]]; then
      if [[ -d "/etc/wwps/v2ray/conf" && -f "/etc/wwps/v2ray/conf/02_VLESS_TCP_inbounds.json" ]]; then
        configPath=/etc/wwps/v2ray/conf/
        if grep </etc/wwps/v2ray/conf/02_VLESS_TCP_inbounds.json -q '"security": "tls"'; then
          coreInstallType=2
          ctlPath=/etc/wwps/v2ray/v2ctl
        fi
      fi
    fi

    if [[ -d "/etc/wwps/wwps-core" && -f "/etc/wwps/wwps-core/wwps-core" ]]; then
      # Detect wwps-core here
    ...
    if [[ -d "/etc/wwps/hysteria" && -f "/etc/wwps/hysteria/hysteria" ]]; then
      # Detect hysteria here
      if [[ -d "/etc/wwps/hysteria/conf" ]] && [[ -f "/etc/wwps/hysteria/conf/config.json" ]] && [[ -f "/etc/wwps/hysteria/conf/client_network.json" ]]; then
        hysteriaConfigPath=/etc/wwps/hysteria/conf/
      fi
    fi

    if [[ -d "/etc/wwps/tuic" && -f "/etc/wwps/tuic/tuic" ]]; then
      if [[ -d "/etc/wwps/tuic/conf" ]] && [[ -f "/etc/wwps/tuic/conf/config.json" ]]; then
        tuicConfigPath=/etc/wwps/tuic/conf/
      fi
    fi
  fi
}
...
allowPort() {
  ...
  if systemctl status netfilter-persistent 2>/dev/null | grep -q "active (exited)"; then
    ...
    if ! iptables -L | grep -q "$1/${type}(youugiuhiuh)"; then
      updateFirewalldStatus=true
      iptables -I INPUT -p "${type}" --dport "$1" -m comment --comment "allow $1/${type}(youugiuhiuh)" -j ACCEPT
    fi
...
# Read wwps-core reality configuration
readWwpsCoreRealityConfig() {
  ...
  if [[ -n ${coreInstallType} ]]; then
    if [[ ${coreInstallType} == 1 ]]; then
      if [[ -n $(pgrep -f "wwps-core") ]]; then
        echoContent yellow "\nCore: wwps-core[Running]"
      else
        echoContent yellow "\nCore: wwps-core[not running]"
      fi

    elif [[ ${coreInstallType} == 2 || ${coreInstallType} == 3 ]]; then
      if [[ -n $(pgrep -f "v2ray/v2ray") ]]; then
        echoContent yellow "\nCore: wwps-legacy[Running]"
      else
        echoContent yellow "\nCore: wwps-legacy[not running]"
      fi
    fi
    #Read protocol type
    readInstallProtocolType
...
cleanUp() {
  if [[ $1 == "v2rayClean" ]]; then
    rm -rf "$(find /etc/wwps/v2ray/* | grep -E '(config_full.json|conf)')"
    handleV2Ray stop >/dev/null
    rm -f /etc/systemd/system/v2ray.service
  elif [[ $1 == "xrayClean" ]]; then
    rm -rf "$(find /etc/wwps/wwps-core/* | grep -E '(config_full.json|conf)')"
    handleWwpsCore stop >/dev/null
    rm -f /etc/systemd/system/xray.service

  elif [[ $1 == "v2rayDel" ]]; then
    rm -rf /etc/wwps/v2ray/*
...
mkdirTools() {
  ...
  mkdir -p /etc/wwps/v2ray/conf
  mkdir -p /etc/wwps/v2ray/tmp
  mkdir -p /etc/wwps/wwps-core/conf
  mkdir -p /etc/wwps/wwps-core/tmp
  mkdir -p /etc/wwps/hysteria/conf
  mkdir -p /etc/systemd/system/
  mkdir -p /tmp/wwps-tls/

  mkdir -p /etc/wwps/warp

  mkdir -p /etc/wwps/tuic/conf
}
...
initTLSNginxConfig() {
  ...
  if [[ -n ${currentHost} ]]; then
    ...
    if [[ ${historyDomainStatus} == "y" ]]; then
      ...
      echo
      echoContent yellow "Please enter the domain name to be configured: www.google.com --->"
...
acmeInstallSSL() {
  if [[ ${dnsSSLStatus} == "true" ]]; then
    ...
    sudo "$HOME/.acme.sh/acme.sh" --issue -d "*.${dnsTLSDomain}" -d "${dnsTLSDomain}" --dns --yes-I-know-dns-manual-mode-enough-go-ahead-please -k ec-256 --server "${sslType}" ${installSSLIPv6} 2>&1 | tee -a /etc/wwps/tls/acme.log >/dev/null

    local txtValue=
    txtValue=$(tail -n 10 /etc/wwps/tls/acme.log | grep "TXT value" | awk -F "'" '{print $2}')
    if [[ -n ${txtValue} ]]; then
      ...
      if [[ ${addDNSTXTRecordStatus} == "y" ]]; then
        ...
        if echo "${txtAnswer}" | grep -q "^${txtValue}"; then
          ...
          if [[ -n ${installSSLIPv6} ]]; then
            sudo "$HOME/.acme.sh/acme.sh" --renew -d "*.${dnsTLSDomain}" -d "${dnsTLSDomain}" --yes-I-know-dns-manual-mode-enough-go-ahead-please --ecc --server "${sslType}" ${installSSLIPv6} 2>&1 | tee -a /etc/wwps/tls/acme.log >/dev/null
          else
            sudo "$HOME/.acme.sh/acme.sh" --renew -d "*.${dnsTLSDomain}" -d "${dnsTLSDomain}" --yes-I-know-dns-manual-mode-enough-go-ahead-please --ecc --server "${sslType}" 2>&1 | tee -a /etc/wwps/tls/acme.log >/dev/null
          fi
        else
    ...
    echoContent green " ---> Generating certificate"
    sudo "$HOME/.acme.sh/acme.sh" --issue -d "${tlsDomain}" --standalone -k ec-256 --server "${sslType}" ${installSSLIPv6} 2>&1 | tee -a /etc/wwps/tls/acme.log >/dev/null
  fi
}
# Custom port
customPortFunction() {
...
installTLS() {
  ...
  # Install tls
  if [[ -f "/etc/wwps/tls/${tlsDomain}.crt" && -f "/etc/wwps/tls/${tlsDomain}.key" && -n $(cat "/etc/wwps/tls/${tlsDomain}.crt") ]] || [[ -d "$HOME/.acme.sh/${tlsDomain}_ecc" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.key" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.cer" ]]; then
...
nginxBlog() {
  ...
  if [[ -d ${nginxStaticPath} && -f "${nginxStaticPath}/check" ]]; then
    ...
    if [[ ${nginxBlogInstallStatus} == "y" ]]; then
      ...
      wget -q -P "${nginxStaticPath}" https://raw.githubusercontent.com/SKIPPINGpetticoatconvent/v2ray-agent/master/fodder/blog/unable/html${randomNum}.zip >/dev/null
...
installV2Ray() {
  ...
  if [[ ${coreInstallType} != "2" && ${coreInstallType} != "3" ]]; then
    ...
    echoContent green " ---> wwps-legacy version:${version}"
    # if wget --help | grep -q show-progress; then
    wget -c -q "${wgetShowProgressStatus}" -P /etc/wwps/v2ray/ "https://github.com/v2fly/v2ray-core/releases/download/${version}/${v2rayCoreCPUVendor}.zip"
    #else
    # wget -c -P /etc/wwps/v2ray/ "https://github.com/v2fly/v2ray-core/releases/download/${version}/${v2rayCoreCPUVendor}.zip" >/dev/ null 2>&1
    # fi

    unzip -o "/etc/wwps/v2ray/${v2rayCoreCPUVendor}.zip" -d /etc/wwps/v2ray >/dev/null
    rm -rf "/etc/wwps/v2ray/${v2rayCoreCPUVendor}.zip"
  else
    if [[ ${selectCoreType} == "3" ]]; then
      echoContent green " ---> Lock wwps-legacy version to v4.32.1"
      rm -f /etc/wwps/v2ray/v2ray
      rm -f /etc/wwps/v2ray/v2ctl
      installV2Ray "$1"
    else
      echoContent green " ---> v2ray-core version:$(/etc/wwps/v2ray/v2ray --version | awk '{print $2}' | head -1)"
...
installHysteria() {
  ...
  if [[ -z ${hysteriaConfigPath} ]]; then
    ...
    wget -c -q "${wgetShowProgressStatus}" -P /etc/wwps/hysteria/ "https://github.com/apernet/hysteria/releases/download/${version}/${hysteriaCoreCPUVendor}"
    mv "/etc/wwps/hysteria/${hysteriaCoreCPUVendor}" /etc/wwps/hysteria/hysteria
    chmod 655 /etc/wwps/hysteria/hysteria
  else
    echoContent green " ---> Hysteria version:$(/etc/wwps/hysteria/hysteria --version | awk '{print $3}')"
    read -r -p "Would you like to update or upgrade? [y/n]:" reInstallHysteriaStatus
    if [[ ${reInstallHysteriaStatus} == "y" ]]; then
      rm -f /etc/wwps/hysteria/hysteria
      installHysteria "$1"
    fi
  fi
...
installTuic() {
  ...
  if [[ -z ${tuicConfigPath} ]]; then
    ...
    wget -c -q "${wgetShowProgressStatus}" -P /etc/wwps/tuic/ "https://github.com/EAimTY/tuic/releases/download/${version}/${version}${tuicCoreCPUVendor}"
    mv "/etc/wwps/tuic/${version}${tuicCoreCPUVendor}" /etc/wwps/tuic/tuic
    chmod 655 /etc/wwps/tuic/tuic
  else
    echoContent green " ---> Tuic version:$(/etc/wwps/tuic/tuic -v)"
    read -r -p "Would you like to update or upgrade? [y/n]:" reInstallTuicStatus
    if [[ ${reInstallTuicStatus} == "y" ]]; then
      rm -f /etc/wwps/tuic/tuic
      tuicConfigPath=
      installTuic "$1"
    fi
  fi
...
installWwpsCore() {
  ...
  if [[ ${coreInstallType} != "1" ]]; then
    ...
    echoContent green " ---> wwps-core version:$(/etc/wwps/wwps-core/wwps-core --version | awk '{print $2}' | head -1)"
    read -r -p "Would you like to update or upgrade? [y/n]:" reInstallWwpsCoreStatus
...
v2rayVersionManageMenu() {
  echoContent skyBlue "\nProgress$1/${totalProgress}: wwps-legacy version management"
  if [[ ! -d "/etc/wwps/v2ray/" ]]; then
    echoContent red " ---> The installation directory is not detected, please execute the script to install the content"
    menu
    exit 0
  fi
  echoContent red "\n================================================ ================="
  echoContent yellow "1.Upgrade wwps-legacy"
  echoContent yellow "2.Fallback wwps-legacy"
  echoContent yellow "3.Close wwps-legacy"
  echoContent yellow "4.Open wwps-legacy"
  echoContent yellow "5.Restart wwps-legacy"
...
# wwps-core version management
wwpsCoreVersionManageMenu() {
  echoContent skyBlue "\nProgress$1/${totalProgress}: wwps-core version management"
  if [[ ! -d "/etc/wwps/wwps-core/" ]]; then
    echoContent red " ---> The installation directory is not detected, please execute the script to install the content"
    menu
    exit 0
  fi
  echoContent red "\n================================================ ================="
  echoContent yellow "1.Upgrade wwps-core"
  echoContent yellow "2.Upgrade wwps-core preview version"
  echoContent yellow "3.Fallback wwps-core"
  echoContent yellow "4.Close wwps-core"
  echoContent yellow "5.Open wwps-core"
  echoContent yellow "6.Restart wwps-core"
  echoContent yellow "7.Update geosite, geoip"
  echoContent yellow "8.Set up automatic update of geo files [updated every morning]"
  echoContent red "================================================== ==============="
  read -r -p "Please select:" selectWwpsCoreType
  if [[ ${selectWwpsCoreType} == "1" ]]; then
    ...
    echoContent skyBlue "------------------------------------------------- ---------------"
    read -r -p "Please enter the version you want to roll back:" selectWwpsCoreVersionType
...
updateV2Ray() {
  ...
  if [[ -z ${coreInstallType} ]]; then
    ...
    echoContent green " ---> Current v2ray-core version: $(/etc/wwps/v2ray/v2ray --version | awk '{print $2}' | head -1)"
    ...
    if [[ -n $1 ]]; then
      ...
      if [[ ${rollbackV2RayStatus} == "y" ]]; then
        if [[ ${coreInstallType} == "2" ]]; then
          ...
          echoContent green " ---> Current wwps-core version: $(/etc/wwps/wwps-core/wwps-core --version | awk '{print $2}' | head -1)"
        fi
    ...
    elif [[ ${version} == "v$(/etc/wwps/v2ray/v2ray --version | awk '{print $2}' | head -1)" ]]; then
...
updateWwpsCore() {
  ...
  if [[ -z ${coreInstallType} ]]; then
    ...
    if [[ -n $1 ]]; then
      read -r -p "The rollback version is ${version}, do you want to continue? [y/n]:" rollbackWwpsCoreStatus
      ...
      read -r -p "The current version is the same as the latest version. Do you want to reinstall? [y/n]:" reInstallWwpsCoreStatus
      if [[ ${reInstallWwpsCoreStatus} == "y" ]]; then
        handleWwpsCore stop
        rm -f /etc/wwps/wwps-core/wwps-core
        rm -f /etc/wwps/wwps-core/wwps-core
      ...
      read -r -p "The latest version is: ${version}, is it updated? [y/n]:" installWwpsCoreStatus
...
# wwps-legacy starts automatically after booting
installV2RayService() {
  echoContent skyBlue "\nProgress$1/${totalProgress}: Configure wwps-legacy to start automatically at boot"
  if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
    rm -rf /etc/systemd/system/wwps-legacy.service
    touch /etc/systemd/system/wwps-legacy.service
    execStart='/etc/wwps/v2ray/v2ray -confdir /etc/wwps/v2ray/conf'
    cat <<EOF >/etc/systemd/system/wwps-legacy.service
[Unit]
Description=wwps-legacy Service
Documentation=https://wwps.com
...
EOF
    systemctl daemon-reload
    systemctl enable wwps-legacy.service
    echoContent green " ---> Configure wwps-legacy to start automatically at boot"
  fi
}
...
installHysteriaService() {
  ...
  if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
    ...
    execStart='/etc/wwps/hysteria/hysteria --log-level info -c /etc/wwps/hysteria/conf/config.json server'
...
installTuicService() {
  ...
  if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
    ...
    execStart='/etc/wwps/tuic/tuic -c /etc/wwps/tuic/conf/config.json'
...
# wwps-core starts automatically after booting
installWwpsCoreService() {
  echoContent skyBlue "\nProgress$1/${totalProgress}: Configure wwps-core to start automatically at boot"
  if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
...
EOF
    systemctl daemon-reload
    systemctl enable wwps-core.service
    echoContent green " ---> Configure wwps-core to start automatically at boot"
  fi
}

#Operation wwps-legacy
handleV2Ray() {
  # shellcheck disable=SC2010
  if find /bin /usr/bin | grep -q systemctl && ls /etc/systemd/system/ | grep -q wwps-legacy.service; then
    if [[ -z $(pgrep -f "wwps-legacy") ]] && [[ $1 == "start" ]]; then
      systemctl start wwps-legacy.service
    elif [[ -n $(pgrep -f "wwps-legacy") ]] && [[ $1 == "stop" ]]; then
      systemctl stop wwps-legacy.service
    fi
  fi
  sleep 0.8

  if [[ $1 == "start" ]]; then
    if [[ -n $(pgrep -f "wwps-legacy") ]]; then
      echoContent green " ---> wwps-legacy started successfully"
    else
      echoContent red "wwps-legacy failed to start"
      echoContent red "Please manually execute [/etc/wwps/v2ray/v2ray -confdir /etc/wwps/v2ray/conf] and check the error log"
      exit 0
    fi
  elif [[ $1 == "stop" ]]; then
    if [[ -z $(pgrep -f "wwps-legacy") ]]; then
      echoContent green " ---> wwps-legacy closed successfully"
    else
      echoContent red "wwps-legacy failed to close"
      echoContent red "Please execute manually [ps -ef|grep -v grep|grep wwps-legacy|awk '{print \$2}'|xargs kill -9]"
      exit 0
    fi
  fi
}
...
handleHysteria() {
  ...
  if [[ $1 == "start" ]]; then
    if [[ -n $(pgrep -f "hysteria/hysteria") ]]; then
      ...
      echoContent red "Please manually execute [/etc/wwps/hysteria/hysteria --log-level debug -c /etc/wwps/hysteria/conf/config.json server] to view the error log"
      exit 0
    fi
  elif [[ $1 == "stop" ]]; then
...
handleTuic() {
  ...
  if [[ $1 == "start" ]]; then
    if [[ -n $(pgrep -f "tuic/tuic") ]]; then
      ...
      echoContent red "Please manually execute [/etc/wwps/tuic/tuic -c /etc/wwps/tuic/conf/config.json] and check the error log"
      exit 0
    fi
  elif [[ $1 == "stop" ]]; then
...
handleWwpsCore() {
  ...
  if [[ $1 == "start" ]]; then
    if [[ -n $(pgrep -f "wwps-core") ]]; then
      echoContent green " ---> wwps-core started successfully"
    else
      echoContent red "wwps-core startup failed"
      echoContent red "Please manually execute the following command [/etc/wwps/wwps-core/wwps-core -confdir /etc/wwps/wwps-core/conf] and feedback the error log"
      exit 0
    fi
  elif [[ $1 == "stop" ]]; then
    if [[ -z $(pgrep -f "wwps-core") ]]; then
      echoContent green " ---> wwps-core closed successfully"
    else
      echoContent red "wwps-core failed to close"
      echoContent red "Please execute manually [ps -ef|grep -v grep|grep wwps-core|awk '{print \$2}'|xargs kill -9]"
      exit 0
    fi
  fi
}
...
initHysteriaNetwork() {
  ...
  cat <<EOF >/etc/wwps/hysteria/conf/client_network.json
{
...
hysteriaPortHopping() {
  ...
  if [[ -z ${hysteriaPortHoppingRange} ]]; then
    ...
    if [[ -z ${portStart} || -z ${portEnd} ]]; then
      ...
      # hysteriaPortHopping
      #else
      iptables -t nat -A PREROUTING -p udp --dport "${portStart}:${portEnd}" -m comment --comment "SKIPPINGpetticoatconvent_portHopping" -j DNAT --to-destination :"${hysteriaPort}"

      if iptables-save | grep -q "SKIPPINGpetticoatconvent_portHopping"; then
...
readHysteriaPortHopping() {
  if [[ -n ${hysteriaPort} ]]; then
    ...
    if iptables-save | grep -q "SKIPPINGpetticoatconvent_portHopping"; then
      portHopping=
      portHopping=$(iptables-save | grep "SKIPPINGpetticoatconvent_portHopping" | cut -d " " -f 8)
...
deleteHysteriaPortHoppingRules() {
  iptables -t nat -L PREROUTING --line-numbers | grep "SKIPPINGpetticoatconvent_portHopping" | awk '{print $1}' | while read -r line; do
...
initHysteriaConfig() {
  ...
  cat <<EOF >/etc/wwps/hysteria/conf/config.json
{
	"listen": ":${hysteriaPort}",
	"protocol": "${hysteriaProtocol}",
	"disable_udp": false,
	"cert": "/etc/wwps/tls/${currentHost}.crt",
	"key": "/etc/wwps/tls/${currentHost}.key",
	"auth": {
		"mode": "passwords",
		"config": []
	},
...
EOF
  ...
  addClientsHysteria "/etc/wwps/hysteria/conf/config.json" true
...
tuicCoreInstall() {
  if ! echo "${currentInstallProtocolType}" | grep -q "0" || [[ -z ${coreInstallType} ]]; then
    echoContent red "\n ---> Due to environmental dependencies, if you install Tuic, please install wwps-core's VLESS_TCP_TLS_Vision first"
    exit 0
  fi
  totalProgress=5
  installTuic 1
...
initV2RayConfig() {
  ...
  if [[ -n ${currentUUID} && -z ${uuid} ]]; then
    ...
    if [[ ${historyUUIDStatus} == "y" ]]; then
      ...
      uuid=$(/etc/wwps/v2ray/v2ctl uuid)
    fi
  elif [[ -z ${uuid} ]]; then
    uuid=$(/etc/wwps/v2ray/v2ctl uuid)
  fi
  ...
  movePreviousConfig
  # log
  cat <<EOF >/etc/wwps/v2ray/conf/00_log.json
{
  "log": {
    "error": "/etc/wwps/v2ray/error.log",
    "loglevel": "warning"
  }
}
EOF
  # outbounds
  if [[ -n ${pingIPv6} ]]; then
    cat <<EOF >/etc/wwps/v2ray/conf/10_ipv6_outbounds.json
{
    "outbounds": [
        {
          "protocol": "freedom",
...
EOF
    ...
    cat <<EOF >/etc/wwps/v2ray/conf/10_ipv4_outbounds.json
{
    "outbounds":[
        {
            "protocol":"freedom",
...
EOF
  ...
  # dns
  cat <<EOF >/etc/wwps/v2ray/conf/11_dns.json
{
    "dns": {
        "servers": [
          "localhost"
...
EOF
  ...
  if echo "${selectCustomInstallType}" | grep -q 4 || [[ $1 == "all" ]]; then
    ...
    cat <<EOF >/etc/wwps/v2ray/conf/04_trojan_TCP_inbounds.json
{
"inbounds":[
	{
	  "port": 31296,
...
EOF
    addClients "/etc/wwps/v2ray/conf/04_trojan_TCP_inbounds.json" "${addClientsStatus}"
  fi
  ...
  if echo "${selectCustomInstallType}" | grep -q 1 || [[ $1 == "all" ]]; then
    ...
    cat <<EOF >/etc/wwps/v2ray/conf/03_VLESS_WS_inbounds.json
{
"inbounds":[
    {
	  "port": 31297,
...
EOF
    addClients "/etc/wwps/v2ray/conf/03_VLESS_WS_inbounds.json" "${addClientsStatus}"
  fi
  ...
  if echo "${selectCustomInstallType}" | grep -q 2 || [[ $1 == "all" ]]; then
    ...
    cat <<EOF >/etc/wwps/v2ray/conf/04_trojan_gRPC_inbounds.json
{
    "inbounds": [
        {
            "port": 31304,
...
EOF
    addClients "/etc/wwps/v2ray/conf/04_trojan_gRPC_inbounds.json" "${addClientsStatus}"
  fi
  ...
  if echo "${selectCustomInstallType}" | grep -q 3 || [[ $1 == "all" ]]; then
    ...
    cat <<EOF >/etc/wwps/v2ray/conf/05_VMess_WS_inbounds.json
{
"inbounds":[
{
  "listen": "127.0.0.1",
...
EOF
    addClients "/etc/wwps/v2ray/conf/05_VMess_WS_inbounds.json" "${addClientsStatus}"
  fi

  if echo "${selectCustomInstallType}" | grep -q 5 || [[ $1 == "all" ]]; then
    getClients "${configPath}../tmp/06_VLESS_gRPC_inbounds.json" "${addClientsStatus}"
    cat <<EOF >/etc/wwps/v2ray/conf/06_VLESS_gRPC_inbounds.json
{
    "inbounds":[
    {
        "port": 31301,
...
EOF
    addClients "/etc/wwps/v2ray/conf/06_VLESS_gRPC_inbounds.json" "${addClientsStatus}"
  fi
  ...
  cat <<EOF >/etc/wwps/v2ray/conf/02_VLESS_TCP_inbounds.json
{
"inbounds":[
{
  "port": ${defaultPort},
...
EOF
  addClients "/etc/wwps/v2ray/conf/02_VLESS_TCP_inbounds.json" "${addClientsStatus}"
...
EOF
  ...
  # outbounds
  if [[ ! -f "/etc/wwps/wwps-core/conf/10_ipv6_outbounds.json" ]]; then
    if [[ -n ${pingIPv6} ]]; then
      cat <<EOF >/etc/wwps/wwps-core/conf/10_ipv6_outbounds.json
{
    "outbounds": [
        {
          "protocol": "freedom",
...
EOF
      ...
      cat <<EOF >/etc/wwps/wwps-core/conf/10_ipv4_outbounds.json
{
    "outbounds":[
        {
            "protocol":"freedom",
...
EOF
  ...
  # routing
  if [[ ! -f "/etc/wwps/wwps-core/conf/09_routing.json" ]]; then
...
EOF
  ...
  if echo "${selectCustomInstallType}" | grep -q 2 || [[ $1 == "all" ]]; then
    ...
    cat <<EOF >/etc/wwps/wwps-core/conf/04_trojan_gRPC_inbounds.json
{
    "inbounds": [
        {
            "port": 31304,
...
EOF
    ...
    rm /etc/wwps/wwps-core/conf/04_trojan_gRPC_inbounds.json >/dev/null 2>&1
  fi
...
EOF
    ...
    cat <<EOF >/etc/wwps/wwps-core/conf/08_VLESS_reality_fallback_grpc_inbounds.json
{
  "inbounds": [
    {
      "port": 31305,
...
EOF
    ...
    rm /etc/wwps/wwps-core/conf/08_VLESS_reality_fallback_grpc_inbounds.json >/dev/null 2>&1
  fi
  installSniffing
}
...
EOF
    ...
    qrCodeBase64Default=$(echo -n "{\"port\":${currentDefaultPort},\"ps\":\"${email}\",\"tls\":\"tls\",\"id\":\"${id}\",\"aid\":0,\"v\":2,\"host\":\"${currentHost}\",\"type\":\"none\",\"path\":\"/${currentPath}vws\",\"net\":\"ws\",\"add\":\"${add}\",\"method\":\"none\",\"peer\":\"${currentHost}\",\"sni\":\"${currentHost}\"}" | base64 -w 0)
    qrCodeBase64Default="${qrCodeBase64Default// /}"

    echoContent yellow " ---> Universal json(VMess+WS+TLS)"
    echoContent green "    {\"port\":${currentDefaultPort},\"ps\":\"${email}\",\"tls\":\"tls\",\"id\":\"${id}\",\"aid\":0,\"v\":2,\"host\":\"${currentHost}\",\"type\":\"none\",\"path\":\"/${currentPath}vws\",\"net\":\"ws\",\"add\":\"${add}\",\"method\":\"none\",\"peer\":\"${currentHost}\",\"sni\":\"${currentHost}\"}\n"
...
EOF
    ...
    cat <<EOF >"/etc/wwps/hysteria/conf/client.json"
{
...
EOF
    local v2rayNConf=
    v2rayNConf="$(cat /etc/wwps/hysteria/conf/client.json)"
...
EOF
    ...
    cat <<EOF >"/etc/wwps/tuic/conf/v2rayN.json"
{
    "relay": {
...
EOF
    local v2rayNConf=
    v2rayNConf="$(cat /etc/wwps/tuic/conf/v2rayN.json)"
    echoContent green "${v2rayNConf}"

    cat <<EOF >>"/etc/wwps/subscribe_local/clashMeta/${email}"
  - name: "${email}_tuic"
    server: ${currentHost}
    type: tuic
    port: ${tuicPort}
...
updateNginxBlog() {
  ...
  if ! echo "${currentInstallProtocolType}" | grep -q "0" || [[ -z ${coreInstallType} ]]; then
    echoContent red "\n ---> Due to environmental dependencies, please install wwps-core's VLESS_TCP_TLS_Vision first"
    exit 0
  fi
  echoContent red "================================================== =========== ===="
  ...
  if [[ ${selectInstallNginxBlogType} =~ ^[1-9]$ ]]; then
    ...
    wget -q -P "${nginxStaticPath}" "https://raw.githubusercontent.com/youugiuhiuh/Wuthering_Waves_Private_Server/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip" >/dev/null
...
unInstall() {
  ...
  if [[ ${coreInstallType} == "1" ]]; then
    handleWwpsCore stop
    rm -rf /etc/systemd/system/wwps-core.service
    echoContent green " ---> Delete wwps-core and it will start automatically after booting"
  ...
  #rm -rf /tmp/wwps-tls/*
  # if [[ -d "/etc/wwps/tls" ]] && [[ -n $(find /etc/wwps/tls/ -name "*.key") ]] && [[ -n $(find /etc/wwps/tls/ -name "*.crt") ]]; then
  # mv /etc/wwps/tls /tmp/wwps-tls
  # if [[ -n $(find /tmp/wwps-tls -name '*.key') ]]; then
  # echoContent yellow " ---> Backup certificate successful, please save it. [/tmp/wwps-tls]"
  #fi
  #fi
  ...
  echoContent green " ---> Uninstall wwps script completed"
}
...
customUUID() {
  ...
  if [[ -z ${currentCustomUUID} ]]; then
    ...
    if [[ -f "/tmp/wwps" && -n $(cat /tmp/wwps) ]]; then
...
# update script
updateV2RayAgent() {
  echoContent skyBlue "\nProgress$1/${totalProgress}: Update wwps script"
  ...
  local version
  version=$(grep 'Current version: v' "/etc/wwps/install.sh" | awk -F "[v]" '{print $2}' | tail -n +2 | head -n 1 | awk -F '["]' '{print $1}')
  ...
  if [[ -f "$HOME/install.sh" ]] && [[ -d "/etc/wwps" ]] && grep <"$HOME/install.sh" -q "作者:youugiuhiuh"; then
    ...
    if [[ -d "/usr/bin/" ]]; then
      if [[ ! -f "/usr/bin/vasma" ]]; then
        ln -s /etc/wwps/install.sh /usr/bin/vasma
      ...
      if [[ ! -f "/usr/sbin/vasma" ]]; then
        ln -s /etc/wwps/install.sh /usr/sbin/vasma
...
setVMessWSRoutingOutbounds() {
  ...
  if [[ -n ${setVMessWSTLSAddress} ]]; then
    ...
    outbounds=$(jq -r ".outbounds += [{\"tag\":\"VMess-out\",\"protocol\":\"vmess\",\"streamSettings\":{\"network\":\"ws\",\"security\":\"tls\",\"tlsSettings\":{},\"wsSettings\":{\"path\":\"${setVMessWSTLSPath}\"}},\"mux\":{\"enabled\":true,\"concurrency\":8},\"settings\":{\"vnext\":[{\"address\":\"${setVMessWSTLSAddress}\",\"port\":${setVMessWSTLSPort},\"users\":[{\"id\":\"${setVMessWSTLSUUID}\",\"security\":\"auto\",\"alterId\":0}]}]}}]" ${configPath}10_ipv4_outbounds.json)
...
# wwps-core personalized installation
...
# Select core installation---v2ray-core, wwps-core
selectCoreInstall() {
  ...
  2)
    v2rayCoreVersion=
    echoContent red " ---> Since v2ray does not support many new features, maintenance is now discontinued in order to reduce development costs. It is recommended to use wwps-core, hysteria, and Tuic"
    exit 0
    ;;
  3)
...
hysteriaCoreInstall() {
  if ! echo "${currentInstallProtocolType}" | grep -q "0" || [[ -z ${coreInstallType} ]]; then
    echoContent red "\n ---> Due to environmental dependencies, if you install hysteria, please install wwps-core's VLESS_TCP_TLS_Vision first"
    exit 0
  fi
  totalProgress=5
  installHysteria 1
...
unInstallWwpsCoreReality() {
  ...
  handleWwpsCore stop
  rm /etc/wwps/wwps-core/conf/07_VLESS_vision_reality_inbounds.json
  rm /etc/wwps/wwps-core/conf/08_VLESS_reality_fallback_grpc_inbounds.json
  echoContent green " ---> Uninstall completed"
}
...
cronFunction() {
  if [[ ${cronName} == "RenewTLS" ]]; then
    ...
    echoContent green " ---> geo update date: $(date "+%F %H:%M:%S")" >>/etc/wwps/crontab_updateGeoSite.log
    exit 0
  fi
}
#Account management
...
addSubscribeMenu() {
  ...
  if [[ ${addSubscribeStatus} == "1" ]]; then
    ...
    rm -rf /etc/wwps/subscribe_remote/clashMeta/*
    rm -rf /etc/wwps/subscribe_remote/default/*
    echo >/etc/wwps/subscribe_remote/remoteSubscribeUrl
...
addOtherSubscribe() {
  ...
  echoContent skyBlue "Input example: www.google.com:443:vps1\n"
  ...
  if [[ -z ${remoteSubscribeUrl} ]]; then
    ...
    if [[ -n $(ls /etc/wwps/subscribe/clashMeta/) || -n $(ls /etc/wwps/subscribe/default/) ]]; then
      find /etc/wwps/subscribe_local/default/* | while read -r email; do
        email=$(echo "${email}" | awk -F "[d][e][f][a][u][l][t][/]" '{print $2}')

        local emailMd5=
        emailMd5=$(echo -n "${email}$(cat "/etc/wwps/subscribe_local/subscribeSalt")"$'\n' | md5sum | awk '{print $1}')
        ...
        if echo "${default}" | grep -q "${email}"; then
          ...
          echo "${default}" >>"/etc/wwps/subscribe_remote/default/${email}"
        ...
        if echo "${clashMetaProxies}" | grep -q "${email}"; then
          ...
          echo "${clashMetaProxies}" >>"/etc/wwps/subscribe_remote/clashMeta/${email}"
  ...
  if echo "${currentInstallProtocolType}" | grep -q 0 && [[ -n ${configPath} ]]; then
    ...
    if [[ -n $(ls /etc/wwps/subscribe_local/default/) ]]; then
      find /etc/wwps/subscribe_local/default/* | while read -r email; do
        ...
        if [[ -f "/etc/wwps/subscribe_remote/default/${email}" ]]; then
          echo >"/etc/wwps/subscribe_remote/default/${email}_tmp"
          while read -r remoteUrl; do
            updateRemoteSubscribe "${emailMd5}" "${email}" "${remoteUrl}" "default"
          done < <(grep "VLESS_TCP/TLS_Vision" <"/etc/wwps/subscribe_remote/default/${email}" | awk -F "@" '{print $2}' | awk -F "?" '{print $1}')

          echo >"/etc/wwps/subscribe_remote/default/${email}"
          cat "/etc/wwps/subscribe_remote/default/${email}_tmp" >"/etc/wwps/subscribe_remote/default/${email}"
          cat "/etc/wwps/subscribe_remote/default/${email}" >>"/etc/wwps/subscribe/default/${emailMd5}"
        fi
        ...
        if [[ -f "/etc/wwps/subscribe_local/clashMeta/${email}" ]]; then
          ...
          if [[ -f "/etc/wwps/subscribe_remote/clashMeta/${email}" ]]; then
            echo >"/etc/wwps/subscribe_remote/clashMeta/${email}_tmp"
            while read -r remoteUrl; do
              updateRemoteSubscribe "${emailMd5}" "${email}" "${remoteUrl}" "ClashMeta"
            done < <(grep -A3 "VLESS_TCP/TLS_Vision" <"/etc/wwps/subscribe_remote/clashMeta/${email}" | awk '/server:|port:/ {print $2}' | paste -d ':' - -)
            echo >"/etc/wwps/subscribe_remote/clashMeta/${email}"
            cat "/etc/wwps/subscribe_remote/clashMeta/${email}_tmp" >"/etc/wwps/subscribe_remote/clashMeta/${email}"
            cat "/etc/wwps/subscribe_remote/clashMeta/${email}" >>"/etc/wwps/subscribe/clashMeta/${emailMd5}"
          fi
...
updateRemoteSubscribe() {
  ...
  local serverAlias=
  serverAlias=$(grep "${remoteDomain}" <"/etc/wwps/subscribe_remote/remoteSubscribeUrl" | awk -F ":" '{print $3}')

  if [[ ${type} == "ClashMeta" ]]; then
    local clashMetaProxies=
    clashMetaProxies=$(curl -s -4 "https://${remoteUrl}/s/clashMeta/${emailMD5}" | sed '/proxies:/d' | sed "s/${email}/${email}_${serverAlias}/g")
    if echo "${clashMetaProxies}" | grep -q "${email}"; then
      echo "${clashMetaProxies}" >>"/etc/wwps/subscribe_remote/clashMeta/${email}_tmp"
    ...
    default=$(curl -s -4 "https://${remoteUrl}/s/default/${emailMD5}" | base64 -d | sed "s/${email}/${email}_${serverAlias}/g")
    if echo "${default}" | grep -q "${email}"; then
      echo "${default}" >>"/etc/wwps/subscribe_remote/default/${email}_tmp"
...
initWwpsCoreRealityConfig() {
  echoContent skyBlue "\nProgress$1/${totalProgress}: Initializing wwps-core reality configuration"
  initRealityPort
  initRealityKey
  initRealityDest
  initRealityClientServersName
...
# wwps-core Reality installation
wwpsCoreRealityInstall() {
  totalProgress=13
  installTools 2
  # Download core
  # prereleaseStatus=true
  #updateWwpsCore
  installWwpsCore 3 false
  # Generate privateKey, configure fallback address, and configure serverNames
  installWwpsCoreService 6
  # initWwpsCoreRealityConfig 5
  #Initialize configuration
  initWwpsCoreConfig custom 7
  handleWwpsCore stop
  cleanUp v2rayClean
  sleep 2
  # start up
...
menu() {
  cd "$HOME" || exit
  echoContent red "\n================================================ ================="
  echoContent green "Author: youugiuhiuh"
