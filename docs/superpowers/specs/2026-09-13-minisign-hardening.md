...
| 路径 | 目标资产 | 本项目是否签名 | 策略 |
|------|---------|---------------|------|
| `system/upgrade.rs` | 自家 `aegis` | ✅ 是（实测 26/27 个 release 有 `.minisig`） | **硬校验**：缺签名即拒绝 |
| `system/core_upgrade.rs` | 上游 `XTLS/Xray-core` | ❌ 否（上游 64 个资产、0 个 minisig） | **条件强校验**：有签名则必须验通过；缺签名则警告后继续，依赖 SHA256 |
...
- `upgrade.rs` 的 `download_minisig` 缺资产/空 URL **有**有效回归测试（实测注入回归会 FAIL）
- 调用点 `?` 传播、使用点 `ok_or_else`、`core_upgrade` 条件判断**无**单测（需网络+完整下载链路）
  → 由 Task 12 的 grep 静态不变量兜底
- 升级路径的**版本精确比较**同样无有效单测（Task 3 已如实标注）
