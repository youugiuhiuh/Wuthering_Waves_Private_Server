...
2. **不误吞消息（有界）**：只有「未验证 + 有 TG 管理员 + 纯数字空白 + >=12 位」才进入比对。注意副作用：**在未验证窗口内**，任何满足该形状的纯数字消息都会被确认路径消费（一致/不一致都回复并短路 `handle_message`）；验证完成后不再发生。其余形状逐条不变。
...
6. **语义限定**：这是 UX 确认（「下次不再出现」），**不是 MITM 防护**——Bot 已把 API 取的码发到 TG，管理员复制 TG 那条也能通过。文档/日志不得表述为「防 MITM」。
7. **落盘并发**：`bot_settings.json` 的读-改-写无锁，单管理员场景可接受；多写路径并发属后续项。
